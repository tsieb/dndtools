...
Start from `docs/README.md` — it is the **map**, not the requirements themselves. The corpus:

| Source | What it carries | ID shape |
|---|---|---|
| `docs/requirements/FEATURE-GAPS.md` | The per-surface inventory of `apps/gm-react`: one row per reachable surface, what it does, its honest limits with evidence anchors that `pnpm feature-audit` verifies, and its e2e specs. | surface names, anchored limits |
| `docs/planning/RC_ROADMAP.md` | The story-level plan to RC1: §1.3 gap findings G1–G21, sixteen workstreams, and the §23 index whose Status column says what landed (with the commit). | `RC-<LANE>-n.m`, `G<n>` |
| `docs/architecture/` | Binding contracts: `ARCHITECTURE.md`, `DATA_MODEL.md`, `NAVIGATION.md` (IA, layers, tiers, top bar, feature tiers), `WIDGETS.md`, `SYSTEM_PACKAGES.md`, `COMBAT_ON_MAP.md`, `SCENE_HISTORY.md`, `PLATFORMS.md` | prose contracts |
| `docs/reference/ICON_VOCABULARY.md`, `docs/design/README.md` | The semantic icon registry; tokens and components. | icon names, token names |
...
| `docs/security/` | The threat model and the Cloud-Enhanced phase-2 review checklist. | checklist items |
...
**Retired sources** (in git history only): the `docs/remake-review/` UX-requirements package, the I1–I21 initiative files (Svelte-era, removed 2026-09-11), and the pre-RC gap-audit passes. Anything citing them is reading a dead path, and that is itself a finding.
