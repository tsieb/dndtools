...
- [ ] Add startup integrity scan for vault metadata/index (`.vault/index.json`) and show actionable repair flow in UI.
- [ ] Add atomic write strategy for markdown notes and index updates (temp file + rename).
- [ ] Add note-level checksums/version markers in frontmatter to detect silent corruption.
- [ ] Add crash-safe journal for pending writes and replay on restart.
- [ ] Add automated backup cadence settings (hourly/daily/manual), local retention policy, and restore UI.
- [ ] Add one-click “Create Safety Snapshot” before high-risk operations (bulk import, migration, mass delete).
- [ ] Add explicit restore UX for accidental deletes beyond current trash flow.
- [ ] Add data-loss regression tests across renderer, Electron, and MCP boundaries.

### Project 1.2: Migration and Schema Evolution (P0)
- [ ] Define schema versioning policy for notes, objects, and `.vault` metadata.
- [ ] Implement migration engine with dry-run mode and rollback checkpoint.
- [ ] Add migration report output (changed files, failures, warnings).
- [ ] Add integration tests for each schema bump against realistic fixture vaults.
- [ ] Add “vault upgrade required” guardrail in desktop bootstrap.

### Project 1.3: Reliability Telemetry and Diagnostics (P1)
- [ ] Add structured error taxonomy (storage, parsing, IPC, MCP sidecar, UI runtime).
- [ ] Add local diagnostics bundle export (logs, runtime status, environment, anonymized metrics).
- [ ] Add MCP sidecar lifecycle telemetry (start/stop/restart/crash reason).
- [ ] Add “last successful sync/index/build” timestamps for critical subsystems.
- [ ] Add in-app “System Health” page in Settings.

## Arc 2: Session-Time UX Excellence

### Project 2.1: Navigation and Information Scent (P1)
- [ ] Add sticky breadcrumbs and contextual location hints for deep folders/linked flows.
- [ ] Add richer sidebar modes: folder tree, recent, favorites, pinned campaign entities.
- [ ] Add keyboard-first command palette expansion (actions + navigation + settings).
- [ ] Add “recently visited” stack with back/forward history clarity.
- [ ] Add quick jump to related notes (same tags, backlinks, same object references).

### Project 2.2: Search and Discovery (P1)
- [ ] Add advanced search operators (`tag:`, `folder:`, `type:`, `updated:`, quoted phrases).
- [ ] Add saved searches and smart collections.
- [ ] Add faceted result filtering panel with counts and clear active-filter chips.
- [ ] Add fuzzy ranking improvements and typo tolerance tuning.
- [ ] Add inline preview snippets for matched sections with jump-to-anchor.
- [ ] Add “search while typing” performance budget monitoring.

### Project 2.3: Reading and Editing Flow (P1)
- [ ] Add split-pane mode (editor + preview) with synchronized scroll.
- [ ] Add better editor insert menus (callouts, tables, templates, embeds, dice blocks).
- [ ] Add unresolved-link workflow improvements (batch create, disambiguation, quick rename).
- [ ] Add richer note metadata editing UI (frontmatter fields as structured controls).
- [ ] Add customizable editor defaults per vault (line wrap, vim, font size, toolbar density).
- [ ] Add session-mode “focus reading” layout with minimal chrome.

### Project 2.4: First-Run and Onboarding (P2)
- [ ] Add guided first-run checklist (create first note, link, tag, search, open settings).
- [ ] Add contextual “why this matters” tips for core concepts (wikilinks, backlinks, object embeds).
- [ ] Add sample D&D vault templates (campaign starter, one-shot, player journal).
- [ ] Add “import from Obsidian” first-run shortcut with safe preview.
- [ ] Add progressive onboarding dismissal + revisit controls.

## Arc 3: Knowledge Model and Domain Depth

### Project 3.1: Objects and Structured Content System (P1)
- [ ] Expand object schema coverage (NPC, location, faction, quest, item, encounter, timeline event).
- [ ] Add object relationship graph (parent/child, ally/enemy, appears-in-session).
- [ ] Add structured editor forms + markdown sync for object-backed notes.
- [ ] Add object validation and linting (missing required fields, broken references).
- [ ] Add object templates with campaign-system variants (5e baseline first).
- [ ] Add object change history and revert.

### Project 3.2: Linking and Graph Intelligence (P1)
- [ ] Add incremental link graph updates from storage events (avoid full rebuild).
- [ ] Add graph quality tools: orphan detection, dead links, high-centrality note insights.
- [ ] Add backlink context snippets in UI and MCP responses.
- [ ] Add alias-aware and disambiguated link resolution.
- [ ] Add optional visual graph exploration view with filters.

### Project 3.3: Templates, Automation, and Reuse (P2)
- [ ] Add global + folder-scoped note templates.
- [ ] Add template variables (date, campaign name, session number, character names).
- [ ] Add “create from template” via command palette and toolbar.
- [ ] Add reusable content blocks/snippets library.
- [ ] Add one-click session recap generator scaffold (non-AI baseline).

## Arc 4: MCP and AI Collaboration Platform

### Project 4.1: MCP Tool Reliability and Contracts (P0)
- [ ] Add strict request/response schemas for all MCP tools/resources.
- [ ] Add comprehensive unit tests for every MCP tool module (notes/search/vault/objects/boards).
- [ ] Add tool-level permission model (read-only, write-staged, write-direct).
- [ ] Add deterministic tool error payload format with actionable remediation hints.
- [ ] Add tool idempotency guidelines and tests for safe retries.

### Project 4.2: Staged Changes and Human Oversight (P1)
- [ ] Expand pending-change preview to include semantic summaries (rename, content delta, link impacts).
- [ ] Add conflict detection for MCP edits vs live UI edits.
- [ ] Add per-agent policy presets (auto-approve read-only, require review for structural edits).
- [ ] Add batch approval filters and diff search within pending changes.
- [ ] Add audit trail for MCP-applied changes (who/what/when/why).

### Project 4.3: Agent-Facing Ergonomics (P2)
- [ ] Add richer vault summary APIs (campaign health, coverage gaps, stale notes).
- [ ] Add task-focused MCP bundles (session prep, recap generation, continuity checks).
- [ ] Add stable URI strategy for resources and discoverability metadata.
- [ ] Add guidance docs/examples for agent prompts and safe operating patterns.
- [ ] Add MCP inspector workflow docs tied to this repo’s architecture.
...
Context:
- Only storage-level MCP test coverage currently exists (`mcp/storage.test.ts`).
Targets:
- `mcp/tools/**/*.test.ts`
