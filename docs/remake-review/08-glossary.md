...
- The Command Center is active in both idle and session-active modes; it is the DM's working
...
**Scope:** Application-wide when active. Session state machine has two modes: `idle` and
`active`.
**Related:** Command Center, Session State, Combat, Player View.
...
**Notes:**
- A session begins when the DM transitions from idle to active mode.
...
**Scope:** Active session context only. Session state is reset or archived at session end.
**Related:** Session, Combat.
...
### Visibility
**Definition:** The DM-controlled property determining whether content appears in a player's
scene at all — independent of whether the player has write permissions on that content, and
enforced at the data/storage layer before any UI render.
**Scope:** All content types (notes, objects, POIs, map layers, widgets).
**Related:** Permission Grant, Player View, DM.
...
- `player-visible` — visible to all players by default
- `shared` — visible to specific players who have an explicit grant
