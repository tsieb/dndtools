...
| --- | --- | --- |
| `ListItem` (core) | Row shell inside `ul`/`ol`. `selected` paints emphasis; `interactive` adds button semantics, `aria-pressed` and Enter/Space activation through `onSelect`. Keep nested controls in noninteractive rows. `<ListItem>…</ListItem>` | ScenesCreator, existing scene rows |
