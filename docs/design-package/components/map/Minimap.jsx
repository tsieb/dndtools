 ...
 */
export function Minimap({ viewport = { x: 0.3, y: 0.25, w: 0.34, h: 0.32 }, collapsed: controlled, defaultCollapsed = false, onToggle, onJump, thumb, width = 168, aspect = 1.4, style, ...rest }) {
	const [internal, setInternal] = React.useState(defaultCollapsed);
	const collapsed = controlled != null ? controlled : internal;
	const toggle = () => { const v = !collapsed; if (controlled == null) setInternal(v); onToggle && onToggle(v); };

	if (collapsed) {
		return (
			<button type="button" aria-label="Expand minimap" title="Expand minimap" onClick={toggle} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border-strong)', background: 'var(--color-surface-overlay)', color: 'var(--color-text-secondary)', boxShadow: 'var(--shadow-md)', cursor: 'pointer', ...style }} {...rest}>
	...
	return (
		<div role="img" aria-label="Minimap — current viewport highlighted" style={{ width, borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border-strong)', background: 'var(--color-surface-overlay)', boxShadow: 'var(--shadow-md)', overflow: 'hidden', ...style }} {...rest}>
			<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px var(--space-2)', borderBottom: '1px solid var(--color-border)' }}>
				<span style={{ fontFamily: 'var(--font-sans)', fontSize: 'var(--text-2xs)', letterSpacing: 'var(--tracking-wide)', textTransform: 'uppercase', color: 'var(--color-text-tertiary)' }}>Minimap</span>
