...
12. **Primary paths outrank platform depth (§1.6 D7).** The golden-path journeys (RC-ENG-8.1) stay
    green on every candidate that touches a primary surface: screens, Session, Characters, Maps,
    Notes, onboarding, the shell. An element of a first-party screen is a widget built on the public
    definition surface (a template or the host API), never a private body the builder cannot express
    (RC-WID-5.5 enforces it). Extension features reach primary screens as widgets, not as new bespoke
    chrome.
...
| G14 | Mostly closed. Prod promoted (`v0.3.7`, 2026-09-09), Stripe test mode live on dev with ADR-027 Accepted, TURN TLS + rotation runbook, analytics, merge sync, keyless access (gated), host/join, companion parity, inbox, listing kinds, creator tooling. | Owner steps in §24 (SES, Stripe prod, FCM, TURN prod cutover); RC-CLD-2.2 re-scoped to closing the phase-2 gate honestly, with the Cloud-Enhanced platform itself deferred to epic CLD-6 (post-RC1); RC-CLD-4.5, 4.4; RC-CLD-2.7, 2.8 (new). |
...
### 1.6 Direction set on 2026-09-12

A UX review of the shell and the live-play path (screenshots of `/`, `/board`, `/session` and first
run at desktop and phone width) led the owner to set seven directions. They add CAN-5.4, CAN-5.5,
CAN-6.4, epic CAN-7, epic WID-5, epic SES-6, epic UX-5 and epic ENG-8, rewrite UX-3.6 and UX-3.7,
and add milestone M2.5. Stories cite them as D1–D7.

| #   | Direction                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Where it lands                        |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- |
| D1  | **Screens replace the three run surfaces.** Command Center, the GM screen board and Session overlap because they are one feature. A screen is a user-created, switchable workspace of widgets, kept in a library like other vault items and pinnable to the sidebar. The Command Center becomes the default screen, built from standard widgets, and must look and behave exactly as it does today. Where a standard widget cannot match it, the widget builder is extended until one can. | ADR-041 (CAN-7.1), CAN-7.2–7.9, WID-5 |
| D2  | **Standby permits everything.** Go live is a posture and logging switch. Only the elapsed session clock, automations and session-start triggers wait for it.                                                                                                                                                                                                                                                                                                                               | SES-6.1, SES-6.2                      |
| D3  | **Onboarding is short and tiered.** The demo vault leaves onboarding and becomes an entry in a vault switcher. Beginner and Standard create a Cloud-Enhanced vault without asking; Private (E2EE) is offered at Expert (amends ADR-026). A feature complexity map decides what each tier shows in Settings.                                                                                                                                                                                | UX-5.1–5.4, UX-3.6, UX-3.7            |
| D4  | **Visibility badges by exception.** Tiles and cards are badged only when players can see them; a setting, off by default, marks GM-only items everywhere.                                                                                                                                                                                                                                                                                                                                  | CAN-6.4                               |
| D5  | **Phones keep every feature.** A screen on a phone is navigable so nothing is unreachable or clipped past recovery.                                                                                                                                                                                                                                                                                                                                                                        | CAN-5.4, CAN-5.5, CAN-7.7             |
| D6  | **Navigation stays as it is until a review before release.**                                                                                                                                                                                                                                                                                                                                                                                                                               | UX-5.5                                |
| D7  | **Primary paths first.** Extensibility continues, but first-party screens are built on the public widget platform, and an early-access milestone (M2.5) holds the primary paths to golden-path journeys.                                                                                                                                                                                                                                                                                   | §0.3 rule 12, ENG-8, M2.5             |
...
2. Every "honest limit" left in the UI is one of the three **external** blockers (payment processor,
   signed desktop certificates, curation service) or the one **deferred capability** (Cloud-Enhanced
   phase 2, epic CLD-6) — nothing else. The deferral is a scope decision of this plan, not an outside
   dependency, and the copy for it says the feature is not in this edition rather than "coming soon".
...
| Security      | `pnpm security:audit`, `security:secrets`, the phase-2 gate test (RC-CLD-2.2) | no high; the Cloud-Enhanced decision record ships unapproved and the gate is proven unbypassable               |
...
**Status 2026-09-12.** §1.6 adds 28 stories and rewrites two. The Screens chain (CAN-7.1 → 7.2 →
7.3/7.7 → WID-5.1–5.3 → CAN-7.6 → 7.8 → 7.9) follows the open CAN stories it shares files with
(CAN-4.4 and CAN-5.1 first), so the CAN lane stays the critical path. SES-6, UX-5.1–5.4, CAN-6.4,
CAN-7.1, CAN-7.5, WID-5.4 and ENG-8.1/8.2/8.4 touch other files and can start at once.
...
| **M2.5 — Early access** | §1.6: CAN-5.4/5.5, CAN-6.4, CAN-7, WID-5, SES-6, UX-3.6/3.7, UX-5.1–5.4, ENG-8, and the primary-path POL stories (1.1, 1.2, 1.4, 1.5, 1.8, 1.9, 1.11, 1.18, 1.23) | RC-ENG-8.3 passes: golden paths green on five consecutive runs on both profiles, zero high `ux-ui-reviewer` findings on primary routes, a11y gate clean, no error state in default content.    |
...
in CAN, plus the four new P2 engineering stories) **· M3 not started · M4 not started.** M2.5 (early access, §1.6 D7) was added
2026-09-12 and has not started.
...
### Epic WID-5 — Builder parity: first-party screens from user-buildable widgets (P2)

Current state (2026-09-12): the builder writes `template` definitions over eight renderers in
`app/widgets/templates/` (data table, status list, tracker, action panel, scene message, chart, stat
block, form panel) and eight query sources (`WidgetDataQuerySource`: current combatants, visible
characters, selected scene, session state, notes, maps, content objects, binding). Command
descriptors write to `scene`, `session` or `entity`; none can navigate or open a creation flow. The
Command Center and Session are bespoke React, so a GM cannot rebuild any part of them today (§1.6
D1). This epic closes the gaps the CAN-7.5 parity matrix records.

- **RC-WID-5.1 — Navigation and creation intents.** `M` · P2 · Deps: CAN-7.5 · Owns:
  `packages/core/src/state/widget-package-state.ts`, `packages/core/src/schemas/widget-package.ts`,
  `packages/core/src/security/widget-host-api.ts`, `apps/gm-react/src/app/widgets/hostBridge.ts`,
  `apps/gm-react/src/app/widgets/templates/ActionPanel.tsx`,
  `apps/gm-react/src/app/widgetBuilder/CommandsStep.tsx`, `docs/architecture/WIDGETS.md`. A closed
  intent descriptor kind beside commands: open a screen, open an entity (character, map, note, quest
  by id), open an allow-listed in-app route (never a URL), create (scene, screen, character, map,
  note, widget, through the existing creation flows), and open a Settings tab. The host resolves an
  intent after the actor's read gate, so a player's widget cannot open a DM-only entity. Template
  widgets declare intents; custom widgets need a new navigate host permission that trust review
  approves or denies. Acceptance: core tests for the resolver and the permission gate; e2e: a
  builder-made action panel opens a character and starts "New map"; a denied custom widget's intent
  is dropped and audited.
- **RC-WID-5.2 — Query sources for hub screens.** `M` · P2 · Deps: 5.1 · Owns:
  `packages/core/src/state/widget-package-state.ts`, `packages/core/src/schemas/widget-package.ts`,
  `apps/gm-react/src/app/widgets/dataEnvironment.ts`, `apps/gm-react/src/app/widgetBuilder/DataStep.tsx`,
  `apps/gm-react/src/app/widgetBuilder/DataStepBindings.tsx`. Add the sources the parity matrix
  needs, each resolved through the existing actor-scoped reads and honouring the query audience:
  screens (name, visibility, widget count, thumbnail, live flag), vault counts (characters by kind,
  maps, notes, story threads, factions), party (names, avatars, vitals), campaign (name, system
  package, live screen, session workflow), dice history, handouts, rollable tables, quick reference,
  and any row the matrix adds. Acceptance: an isolation test per source proves a player projection
  never carries a DM-only row; the builder's data step lists every source with a live preview.
- **RC-WID-5.3 — Hub template renderers and bare presentation.** `M` · P2 · Deps: 5.2 · Owns:
  `apps/gm-react/src/app/widgets/templates`, `apps/gm-react/src/app/widgetBuilder/LayoutStep.tsx`,
  `apps/gm-react/src/app/canvas/WidgetFrame.tsx`, `packages/core/src/state/scene-state.ts`. Four new
  template kinds built only from the DS components and tokens: hero (title, subtitle, avatar stack,
  one primary intent), card grid (thumbnail, status badge, meta line, an intent per card, a trailing
  "new" action), launcher (icon tiles with label and hint), and link list (rows with icon, meta and
  chevron). Each has the same empty, loading, hover, focus and selected states as the bespoke
  Command Center part it replaces. A widget instance gains a framed or bare presentation: bare hides
  the frame's title bar and badge in view mode so a hub widget reads as page content, and shows them
  again in edit mode. Acceptance: each template has a snapshot test against the CAN-7.5 baseline of
  the element it replaces; bare widgets keep a labelled region and a visible focus ring (axe); the
  builder previews all four.
- **RC-WID-5.4 — The design-system kit inside the sandbox.** `M` · P2 · Deps: WID-2.4 · Owns:
  `apps/gm-react/public/widget-host.html`, `apps/gm-react/public/widget-kit.css` (new),
  `apps/gm-react/src/app/widgets/SandboxHost.tsx`, `packages/core/src/security/renderer-isolation.ts`,
  `docs/architecture/WIDGETS.md`. A versioned stylesheet served beside the host document gives a
  custom-html-js widget the first-party look without copying it: button, icon button, card, badge,
  chip, list row, input, select, stat, the focus ring, the density sets and the motion collapse, all
  driven by the theme tokens WID-2.4 forwards. The widget CSP changes only to admit that one
  same-origin stylesheet. Acceptance: the starter library's custom showcase restyled with the kit
  matches the DS Button, Card and Badge snapshots in all three themes; a `/security-review` report is
  linked in the journal; WIDGETS.md documents the kit's class contract.
- **RC-WID-5.5 — Builder parity gate.** `S` · P2 · Deps: 5.3, CAN-7.6 · Owns:
  `apps/gm-react/src/app/widgets/parity.ts` (new), `apps/gm-react/src/app/widgets/builtin/index.tsx`.
  A declared map from every builtin body to the public surface it uses (commands, intents, query
  sources), and a test that fails when a widget on a shipped default screen is not a template or
...
- **RC-CAN-5.4 — Moving around a canvas screen on a phone.** `M` · P2 · Deps: 5.1 · Owns:
  `app/SceneBoardCanvas.tsx`, `app/canvas/ZoomCluster.tsx`, `apps/gm-react/src/app/canvas/PhoneNavigator.tsx` (new),
  `screens/Board.tsx`. Current state (2026-09-12 screenshots): at 390px the board draws the desktop
  layout with text near 5px and runs off the right edge. On the phone tier a canvas screen opens as
  the CAN-5.1 stacked list and offers a Layout view for the spatial arrangement (§1.6 D5). Layout
  never renders body text below 12px; one finger pans both axes with native momentum; a pinch snaps
  between Fit, Comfortable and Detail rather than to free percentages; an overview minimap (tap to
  jump) and a "Jump to tile" sheet list every tile; edge fades show where more content lies; every
  tile opens full screen above the bottom navigation. No feature is dropped: every operate and
  configure action a tile has on desktop is reachable from the list, the Layout view or the
  full-screen tile. Acceptance: e2e at 360×640 and 390×844 reaches every tile by pan and by the jump
  sheet; a test asserts no rendered text below 12px on the phone tier; axe clean on mobile.
- **RC-CAN-5.5 — Tile content never clips past recovery.** `M` · P2 · Deps: ENG-8.1 · Owns:
  `app/canvas/WidgetFrame.tsx`, `app/widgets/WidgetRenderSlot.tsx`, `app/widgets/builtin/NotesBody.tsx`,
  `app/widgets/builtin/InitiativeBody.tsx`, `app/widgets/builtin/ListBody.tsx`. Current state: fixed
  tile heights cut the Prep list mid-line and the Initiative tile's empty copy, and 14px display-face
  titles truncate ("Initiative Tra…", "Quick Referen…") against the rule that Cinzel is for 24px and
  up. A body that outgrows its tile scrolls inside it as a labelled, keyboard-scrollable region with
  an edge fade and a "more" count; a truncated title exposes its full text as a tooltip and in the
  accessible name; tile titles use the sans face; one action grows a tile to fit its content.
  Acceptance: the ENG-8.1 overflow detector reports zero unrecoverable clips on /board, /scene/:id
  and every default screen at the three tiers; the two screenshot cases render whole.
...
- **RC-CAN-6.3 — Board empty states and first-tile onboarding.** `S` · P3 · Deps: DSN-3.1, 4.1 ·
  ...
  Acceptance: e2e.
- **RC-CAN-6.4 — Visibility badges by exception.** `S` · P2 · Owns: `app/canvas/WidgetFrame.tsx`,
  `screens/SceneCardsPanel.tsx`, `ds/components/feedback/VisibilityChip.jsx`,
  `apps/gm-react/src/platform/preferences.ts`, `screens/settings/Appearance.tsx`,
  `docs/design/README.md`. Current state: every widget frame on the board and the scene editor
  carries a "DM only" chip, so the chip that matters, on something players can see, is lost among
  them (§1.6 D4). By default a tile or card is badged only when it is visible beyond the GM
  (Players, Shared, Mixed, with the eye icon and text); GM-only items carry no badge. A device
  preference, "Mark GM-only items" in the package's vocabulary and off by default, restores the
  GM-only chip everywhere for dense shared screens. Authoring controls where visibility is being set
  (map layers, points of interest, handout recipients) keep their explicit per-row state, and the
  preview-as-player overlay (CAN-6.1) is unchanged. The design README's safety-language rule becomes
  "name every visibility that differs from GM-only". Acceptance: unit tests for the badge decision;
  e2e: a GM-only tile shows no chip, a shared tile shows "Players", the preference restores the
  GM-only chips; ES catalog updated.

### Epic CAN-7 — Screens: one customizable home for running the table (P2)

Current state (2026-09-12): `/` is `screens/CommandCenter.tsx`, a bespoke hub (hero, scene tiles,
Create launchers, Manage links, Library counts); `/board` is `screens/Board.tsx`, the home scene
(`commandCenter.homeSceneId`) on the bounded canvas; `/session` is `screens/session/*`, a bespoke
console. Dice, combat and audio appear on all three, and the sidebar's Scenes group already lists
scenes as quick access. §1.6 D1 turns all three into screens: scenes of widgets with a library, a
switcher and sidebar pins, where the Command Center is the default screen and loses nothing it does
today.

- **RC-CAN-7.1 — ADR-041: Screens.** `S` · P2 · Owns: `docs/adr/041-screens-as-the-run-surface.md`
  (new), `docs/adr/README.md`. Record D1 and decide, with the options weighed: a screen is a scene
  (no second entity) carrying screen metadata (pinned, pin order, layout policy, template origin);
  the layout policies are flow (a responsive column grid with auto-height tiles whose reading order
  is the layout order, the default for hub screens) and canvas (today's bounded board and free
  editor); the routes are /screens for the library and /screen/:id, with /, /board, /session,
  /scenes and /scene/:id resolving to screens in one history entry; fresh vaults get three default
  screens (Command Center as home, GM screen, Session), and existing vaults keep their home board
  untouched as the GM screen and gain the Command Center as home; the vocabulary that separates
  player-projected scenes from GM screens; sidebar pins as a user-defined group under ADR-013.
  Acceptance: ADR-041 Accepted with its index row; NAVIGATION.md and WIDGETS.md point at it.
- **RC-CAN-7.2 — Screen metadata, pins and the screens read.** `M` · P2 · Deps: 7.1 · Owns:
  `packages/core/src/state/scene-state.ts`, `packages/core/src/commands/scene.ts`,
  `packages/core/src/commands/scene-meta.ts`, `packages/core/src/queries/screens.ts` (new). Additive
  screen metadata on the scene (a byte-identical round trip when absent, so no schema bump); the
  commands `scene.set-pinned`, `scene.reorder-pins`, `scene.set-layout-policy` and
  `scene.duplicate`; and a `listScreensForActor` read in which players list only the screens visible
  to them. Acceptance: reducer and schema tests; a round-trip test on a fixture vault with a
  customised home board proves every widget survives; an isolation test for the read.
- **RC-CAN-7.3 — The screens library and switcher.** `L` · P2 · Deps: 7.2, 4.4, 5.1 · Owns:
  `apps/gm-react/src/screens/screen` (new), `screens/Board.tsx`, `screens/board`,
  `screens/ScenesCreator.tsx`, `screens/SceneCardsPanel.tsx`, `app/CommandPalette.tsx`. The /screens
  library lists screens the way the other library sections list items: search, sort, tags,
  thumbnails, visibility named honestly ("GM only", "Shared", replacing today's "Draft" and "Ready",
  which are derived from visibility), a live marker, new from template (Command Center, GM screen,
  Session, Prep, Blank, plus the CAN-4.4 templates), duplicate, rename, and delete with undo.
  /screen/:id renders any screen by its layout policy; canvas screens reuse the bounded board engine
  and Edit layout opens the scene editor. A switcher in the screen header and screen actions in the
  command palette jump between screens. /board and /scenes resolve into this route. Acceptance: e2e
  on both profiles creates from a template, renames, pins, and switches by the library, the header
  switcher and the palette; an old /board bookmark opens the GM screen; one history push per switch.
- **RC-CAN-7.4 — Pinned screens in the sidebar, rail and More sheet.** `M` · P2 · Deps: 7.3 · Owns:
  `app/shell/Sidebar.tsx`, `app/shell/RailNav.tsx`, `app/shell/MoreSheet.tsx`, `app/shell/rows.tsx`,
  `app/shell/sections.ts`, `ds/components/navigation/BottomTabBar.jsx`. The sidebar's Scenes group
  becomes Screens: pinned screens in the GM's order, a create button, and "All screens" to the
  library. Pin and unpin from the library, the screen header and a row menu; reorder by drag and by
  keyboard (one command). The live screen carries the session posture. The rail shows pins as icon
  rows with accessible names, and the phone More sheet lists them first. The Run the table entries
  open their default screens; the global IA is otherwise unchanged (§1.6 D6). Acceptance: e2e at the
  three tiers; keyboard reorder dispatches `scene.reorder-pins`; axe `landmark-unique` still passes.
- **RC-CAN-7.5 — Parity inventory of the three run surfaces.** `S` · P2 · Deps: 7.1 · Owns:
  `docs/planning/SCREENS_PARITY.md` (new). Before anything is converted, record every element of the
  Command Center, the board and Session as they ship. Command Center: the hero (campaign name, live
  state, party avatars, the Resume or Open primary), scene tiles (thumbnail, status, widget count),
  Create launchers, Manage links, Library counts, and the player and observer variants. Board: every
  seeded tile. Session: the lifecycle header, combat tracker, encounter builder, dice tray, rollable
  tables, handouts, active map and projection, now playing, campaign date, prep and recap, capture,
  roster and party, schedule, rest timeline. For each element: behaviour, states (empty, loading,
  live, preview-as-player, read-only), keyboard and heading structure, behaviour per tier, and a
  screenshot set (three themes × desktop, rail, phone) in the run journal. Each row maps to an
  existing builtin, a template, or a named gap with a size. Acceptance: the matrix covers every
  control in an aria snapshot of each route; every gap is assigned to WID-5.1–5.4 or filed as a new
  WID-5 story before CAN-7.6 starts.
- **RC-CAN-7.6 — The Command Center as the default screen.** `L` · P2 · Deps: 7.3, 7.5, 7.7,
  WID-5.1, WID-5.2, WID-5.3, ENG-8.2 · Owns: `screens/CommandCenter.tsx`, `screens/SceneCardsPanel.tsx`,
  `screens/SceneQueuePanel.tsx`, `packages/core/src/commands/command-center.ts`,
  `packages/core/src/state/widget-package-state.ts`, `apps/gm-react/src/runtime/demo-seed.ts`. First
  commit the aria and DOM baselines of today's Command Center as vitest snapshots. Then rebuild it
  as the home screen: a flow screen whose hero, scenes, Create, Manage and Library parts are system
  template widgets in bare presentation, defined through the public definition surface. The
  ensure-home command creates it for fresh vaults and adds it as the new home for existing vaults,
  whose board stays as the GM screen. The GM can move, remove, restyle, duplicate or rebuild any part
  in the widget builder, and a rebuilt copy looks and works the same. An empty vault shows
  intentional empty states, never errors. Acceptance: the baseline diff is empty apart from
  documented widget-region wrappers; a `ux-ui-reviewer` comparison of before and after screenshots per
  theme and tier has no regression finding; every CAN-7.5 Command Center row checked; keyboard and
  heading order unchanged; `scene-first-render` inside budget; a GM-built duplicate of each part
  passes the same snapshot.
- **RC-CAN-7.7 — The flow layout policy.** `L` · P2 · Deps: 7.2 · Owns: `apps/gm-react/src/app/canvas/FlowBoard.tsx`
  (new), `app/board-helpers.ts`, `app/SceneBoardModel.ts`, `screens/sceneEditor/index.tsx`. A flow
  screen lays tiles in a responsive column grid (column counts from the viewport tier); heights follow
  content; reading and focus order follow layout order. Moving a tile by drag, by keyboard or from
  its menu dispatches the same move command; resizing picks a column span. The phone tier collapses
  to one column and keeps every control. Acceptance: the Command Center's current two-column
  arrangement reproduces at desktop and reflows at rail and phone; a keyboard reorder e2e; no tile
  clips content at any tier.
- **RC-CAN-7.8 — Session as a screen.** `L` · P2 · Deps: 7.6, SES-6.2, WID-5.5 · Owns:
  `screens/session/index.tsx`, `screens/session/Lifecycle.tsx`, `screens/session/useSessionView.ts`,
  `app/widgets/builtin/SessionBody.tsx`, `app/widgets/builtin/CombatBody.tsx`,
  `app/widgets/builtin/DiceBody.tsx`, `packages/core/src/state/widget-package-state.ts`. The Session
  console becomes the default Session screen. Each CAN-7.5 Session row is a widget: session status
  (the Standby, Prep, Live and Recap control with Go live), combat tracker, dice, rollable tables,
  handouts, stage and projection, now playing, campaign date, prep and recap, capture, party. The
  encounter builder, end-session and rest dialogs open from their widgets as they do today, and
  /session opens this screen. Widgets that stay builtin pass the WID-5.5 gate. Acceptance: every
  Session parity row checked; `combat.spec.ts` and the session and dice specs pass with selector-only
  edits; the quick panel still works on every route; the before and after review as in CAN-7.6.
- **RC-CAN-7.9 — Retire the bespoke run screens.** `S` · P3 · Deps: 7.4, 7.6, 7.8 · Owns:
  `screens/CommandCenter.tsx`, `screens/Board.tsx`, `screens/session/index.tsx`,
  `docs/requirements/FEATURE-GAPS.md`. After one promotion with screens live on the dev stage,
  delete the bespoke code paths the default screens replaced, point the Run the table entries at
  their screens, and update NAVIGATION.md, WIDGETS.md, FEATURE-GAPS and the POL surfaces.
  Acceptance: no route renders a bespoke run screen; the boot bundle does not grow; the full e2e
  suite is green on both profiles.
...
### Epic SES-6 — Standby is fully usable (P2)

Current state (2026-09-12): `SESSION_COMMAND_AVAILABILITY` in
`packages/core/src/lifecycle/session-workflow.ts` puts dice, tables, combat, handouts, map
projection, combat resources and the quick timer in `live-session`, and matching
`workflow !== 'active'` guards in the dice, handout, combat, character-resources, session-control,
session-quick-timer and widget-command commands reject them. In Standby the Session screen therefore
disables dice, combat and handouts ("Go live to…"). §1.6 D2: Go live is a posture and logging
switch.

- **RC-SES-6.1 — Only the clock, automations and start triggers wait for Go live.** `M` · P2 ·
  Owns: `packages/core/src/lifecycle/session-workflow.ts`, `packages/core/src/commands/dice.ts`,
  `packages/core/src/commands/handout.ts`, `packages/core/src/commands/combat.ts`,
  `packages/core/src/commands/character-resources.ts`,
  `packages/core/src/commands/session-control.ts`,
  `packages/core/src/commands/session-quick-timer.ts`,
  `packages/core/src/commands/widget-command.ts`, `packages/core/src/queries/command-actions.ts`,
  `packages/core/src/queries/session-control.ts`, `packages/core/src/state/audio-automation.ts`,
  `apps/gm-react/src/runtime/combat-audio-automation.ts`. Move every live-session and dm-admin
  command to always available, including the quick-panel countdown timer (a table tool, not the
  session clock). What stays live-only: the elapsed session clock, audio automation rules and SFX
  events, and session-start triggers (scene activation and the start-of-session side effects).
  Every roll, combat event and handout records the workflow it happened in (an additive field), so
  the session log, capture and recap include only what happened while live and history labels the
  rest "Outside a session". Acceptance: the lockstep regression test updated; a test per moved
  command in every workflow state; starting combat in Standby does not fire the combat-start
  automation and starting it live does; capture excludes Standby rolls.
- **RC-SES-6.2 — Remove the Go-live gates from the UI.** `M` · P2 · Deps: 6.1 · Owns:
  `screens/session/DiceTray.tsx`, `screens/session/Handouts.tsx`, `screens/session/CombatTracker.tsx`,
  `screens/session/Tables.tsx`, `screens/session/Lifecycle.tsx`, `screens/session/index.tsx`,
  `app/session/QuickPanel.tsx`, `app/widgets/builtin/DiceBody.tsx`, `app/widgets/builtin/TimerBody.tsx`,
  `app/widgets/builtin/CombatBody.tsx`, `app/widgets/builtin/NextTurnControl.tsx`,
  `app/widgets/builtin/InitiativeTracker.tsx`. Every control works in every workflow state. The
  Standby banner becomes a quiet status ("Not recording. Rolls and combat are logged once you start
  the session.") and Go live stays the header's one primary, worded for what it does: it starts the
  log, the clock and the automations. Acceptance: e2e in Standby rolls dice, draws a table, builds an
  encounter, runs two rounds and pushes a handout; no control carries "go live" copy (a catalog
  test); ES updated.
...
day the deferred Cloud-Enhanced platform (epic CLD-6) clears its sign-off.
...
- **RC-AI-4.1 — Copilot client + indexer contract (behind the phase-2 gate).** `M` · P2 · Deps:
  CLD-2.2 (the gate is closed here; switching the feature on waits on CLD-6.5) · Owns: `cloud/copilot.ts`, `packages/cloud-fns/src/copilot/*`, `infra/app-api` (scale-
...
- **RC-CLD-2.2 — Cloud-Enhanced gate closure and honest limit.** `S` · P2 · Owns:
  `docs/security/vault-privacy-modes-threat-model.md`,
  ...
  `apps/gm-react/src/screens/settings/SyncPrivacy.tsx` (copy only).
  **Re-scoped 2026-09-16, replacing the 2026-09-11 re-open.** The story was written as a review of the
  Cloud-Enhanced server path, and six of the seven checklist items in the threat model have nothing to
  review: no server-side mode authority, no server-readable content path, no server-side feature code,
  no mode-transition migration and no purge coverage for derived data exist anywhere in the tree. A
  review cannot produce the system it reviews, and the two failed dispatcher runs before the re-open
  were that mismatch, not a flaky story. The platform is therefore deferred to **epic CLD-6** and the
  checklist becomes that epic's exit criterion — which is what the 2026-09-08 disposition already asked
  of the implementing change. What stays in RC-1 is making the closed gate true, permanent and honestly
  described: the decision record ships unapproved; an approval cannot be manufactured by a caller, only
  by the shipped selector and, later, a server-side registration; the threat model says plainly that
  phase 2 is unbuilt rather than pending; and every Cloud-Enhanced surface states that the transport is
  still end-to-end encrypted and the features are not in this edition. Nothing shipped is blocked by
  the deferral: RC-CLD-2.5 and RC-AI-4.1 already run fail-closed behind the gate. Acceptance: a core
  test proves an approved record that did not come from the shipped selector cannot open a
  server-readable path; the threat model's disposition records the deferral and points every open item
  at CLD-6; no surface promises a capability the gate withholds (e2e); the record stays unapproved and
  no other RC-1 story may flip it.
...
- **RC-CLD-2.5 — Keyless browser access (Cloud-Enhanced).** `M` · P2 · Deps: 2.2. Acceptance: gated e2e.
  **Done, and it stays gated.** The client half ships fail-closed; the browser cannot open a vault
  without the device key until CLD-6 supplies the server side, and the surface says so.
...
### Epic CLD-6 — Cloud-Enhanced phase 2 platform (P4, deferred past RC-1)

**Why this epic exists.** ADR-026 split vault privacy into two modes and phase 1 shipped the whole of
the user-facing half: the mode, the consent moment, the gates, the recovery key. It did not ship the
other half, and no story elsewhere in this plan does either. The server-side capability the paid tiers
were argued on — managed AI over the campaign, server-side search, opening a vault in a browser with
no device key — needs a server that may read vault content, and that is a platform: an authority for
the mode, a content path only that authority can open, transitions and deletion that cannot strand
plaintext, and then the features. Filing it as one `M` security review is what stalled RC-CLD-2.2
twice. It is sized honestly here and placed outside RC-1 so the release criteria stay true.

**Rules for the epic.** The order is fixed — authority, path, lifecycle, features, sign-off — because
each story is the precondition that makes the next reviewable. Nothing in RC-1 depends on any of it;
the two capabilities that would use it (RC-CLD-2.5, RC-AI-4.1) already ship fail-closed, so the epic
can start whenever the product decides to pay for it. Only RC-CLD-6.5 may change the decision record,
and no feature inherits approval from a partial review.

- **RC-CLD-6.1 — Server-side authority for the vault mode.** `L` · P4 · Owns:
  `packages/cloud-fns/src/sync`, `infra/sync-api/template.yaml`. The server, not the device, becomes
  the authority on a vault's privacy mode: the mode is recorded against the authenticated owner when
  consent is given, consulted on every request that could widen the trust boundary, and a missing or
  failed read is a refusal rather than a default. The device-local flag stays what it is today — UX
  state. Acceptance: contract tests show a client-supplied mode is never trusted, an absent
  registration reads as Private, and a read failure refuses; cross-tenant tests cover the store.
- **RC-CLD-6.2 — The gated server-readable content path.** `L` · P4 · Deps: 6.1 · Owns:
  `packages/cloud-fns/src/sync`, `infra/sync-api/template.yaml`, `packages/core/src/security`.
  Content the server may read is accepted only for vaults the authority from 6.1 calls
  Cloud-Enhanced, encrypted at rest under the dedicated per-stage key, and scoped to the
  authenticated owner exactly as the ciphertext path is. A Private vault's content stays unreadable
  whatever the client sends. Acceptance: contract tests reject a Private vault's readable write and
  every client-supplied mode override; a race test proves a concurrent mode change cannot authorise a
  stale write; cross-tenant tests cover every new row, object and key prefix; the Private path's
  metadata invariant (SEC-009) is unchanged.
- **RC-CLD-6.3 — Mode transitions, deletion, and the re-consent rule.** `L` · P4 · Deps: 6.2 · Owns:
  `packages/cloud-fns/src/sync`, `packages/cloud-fns/src/app-api`,
  `apps/gm-react/src/screens/settings/SyncPrivacy.tsx`,
  `docs/adr/042-tiered-vault-privacy-default.md`. Changing a vault's mode is a migration, not a
  flag: the new-mode copy verifies before any old-mode artifact is deleted, an interrupted migration
  leaves the previous usable copy and never claims the switch completed, and moving back to Private
  says plainly that the content was server-readable while the other mode was active. Account and vault
  deletion purge server-readable content and everything derived from it, and a partial failure cannot
  produce a completed-deletion marker. **The re-consent rule:** RC-UX-5.3 makes Cloud-Enhanced the
  default mode for the simpler tiers while the gate is closed, so most vaults will carry that label
  without their owner ever having agreed to server reads. Opening this path must therefore ask those
  vaults again, in this flow, before anything of theirs becomes readable. Acceptance: interrupted
  migration tested in both directions; purge coverage proven for every derived store; a test that an
  existing vault's boundary cannot widen without the new consent; ADR-042 amended with the re-consent
  rule and the migration it finally has.
- **RC-CLD-6.4 — Features on the consented boundary.** `L` · P4 · Deps: 6.3 · Owns:
  `packages/cloud-fns/src/copilot`, `packages/cloud-fns/src/app-api`,
  `apps/gm-react/src/cloud/copilot.ts`. The capabilities the mode was argued for — managed
  Copilot/RAG, server-side search, keyless browser access — read vault content through a read-only
  scoped role, never write derived plaintext into a broader-scoped store, and register every derived
  store with the purge protocol from 6.3. Public marketplace and wiki publication remain a separate
  consent boundary and satisfy nothing here. Acceptance: each feature's role and derived stores
  reviewed and recorded in the threat model; cross-tenant tests per feature; the gated paths of
  RC-CLD-2.5 and RC-AI-4.1 run un-gated in e2e.
- **RC-CLD-6.5 — Phase-2 sign-off and the gate flip.** `M` · P4 · Deps: 6.1–6.4 · Owns:
  `docs/security/vault-privacy-modes-threat-model.md`,
  `packages/core/src/security/cloud-security-decision.ts`, `docs/adr`. The only story allowed to
  approve the record. Every checklist item carries evidence naming a file or a test; the KMS item adds
  the deployed-stage evidence the 2026-09-08 review found outstanding — deployed key policy, an access
  probe, a delivered decrypt audit event — for each stage that serves the path; an independent
  reviewer signs the complete checklist and the owner records that sign-off. Acceptance: no checklist
  item left open; the record is approved in this change and no earlier one; the gated e2e passes
  un-gated on a stage whose evidence is attached; ADR-026 and ADR-042 record the boundary as live.
...
- **RC-UX-3.6 — Onboarding v3: short, tiered and friendly.** `M` · P2 · Deps: 5.2, 5.3 · Owns:
  `app/Onboarding.tsx`, `app/onboarding`, `apps/gm-react/src/cloud/vaultMode.ts`. Rewritten
  2026-09-12 (§1.6 D3); replaces the v2 plan. Today's seven steps (welcome, vault, privacy,
  experience, tools, party, ready) become three at most: first, welcome and "How much do you want on
  screen?" (Beginner, Standard, Expert, each with one plain sentence on what changes); second, your
  table (campaign name, game system, optional party names, optional AI); third, ready, landing on the
  Command Center with one first-action prompt. Beginner and Standard create a Cloud-Enhanced vault
  with a one-line disclosure and a link to Settings; Expert adds the Private (E2EE) or Cloud choice
  with the existing no-recovery acknowledgement. The sample-or-fresh choice leaves onboarding (the
  demo moves to the vault switcher, UX-3.7) and new vaults start empty. Everything is skippable
  except the Expert privacy decision. Acceptance: e2e counts three steps or fewer and reaches the
  Command Center within 60 seconds of scripted input; a Beginner run never renders E2EE copy;
  `onboarding-consent.spec.ts` rewritten to the ADR-042 rules; the e2e suite's seeded fixture vault
  still loads through its test hook.
- **RC-UX-3.7 — The demo campaign as inspiration, from the vault switcher.** `M` · P2 · Deps: 5.4 ·
  Owns: `apps/gm-react/src/runtime/demo-seed.ts`, `apps/gm-react/tests/e2e/fixtures`,
  `apps/gm-react/src/app/shell/VaultSwitcher.tsx` (new). Rewritten 2026-09-12 (§1.6 D3). The vault
  switcher (UX-5.4) gains "Explore the demo campaign": a separate demo vault seeded from the demo
  seed, badged "Demo" wherever the vault name shows, resettable, never synced or backed up, and
  unable to host or join a table. Leaving it returns to the GM's own vault untouched. The seed is
  widened so every surface has real content to borrow from: a system-package switch example, three
  widgets including one custom, a screen with a map tile and a running encounter with tokens, a
  scene package with audio, quests and factions with typed relationships, a calendar with dated
  notes, saved searches, a level-2 character with resources, and one staged assistant proposal.
  Acceptance: e2e opens the demo from the switcher, edits it, resets it, and returns with the GM
  vault's op count unchanged; sync and backup refuse the demo vault (test); the seed still commits as
  one batch with `scene-first-render` in budget; every e2e that relied on the old seed passes.
...
### Epic UX-5 — First run, complexity tiers and vaults (P2)

Current state (2026-09-12): the Beginner, Standard and Expert tier is read only by Settings, where
`TAB_GATE` hides three tabs (Permissions, Plugins, Systems); `FEATURE_GATES` has ten entries, yet
onboarding promises that Beginner hides advanced panels. A device holds one local vault: the
sidebar's campaign chip only navigates home. ADR-026 makes the privacy choice a forced, undefaulted
onboarding step.

- **RC-UX-5.1 — The feature complexity map.** `M` · P2 · Owns:
  `packages/core/src/state/onboarding.ts`, `apps/gm-react/src/screens/settings/shared.tsx`,
  `docs/reference/FEATURE_COMPLEXITY.md` (new). Inventory every user-facing feature and setting:
  each Settings tab and each section inside it, the Extensions and Community areas, the widget
  builder's steps, custom types, system authoring, AI, sync and privacy modes, permissions,
  diagnostics, the map editor's advanced tools. Estimate each one's complexity (core, intermediate,
  advanced) with the reason: what it assumes the GM knows, and what goes wrong if it is misused.
  Encode the result as feature-gate data at section granularity (id, label key, tier, surface,
  section anchor) and generate the reference table from that data. Acceptance: a test enumerates
  every Settings tab and section and fails when one has no gate; a test asserts the reference doc
  matches the data; Private (E2EE) is advanced.
- **RC-UX-5.2 — Tiers decide what Settings shows.** `M` · P2 · Deps: 5.1 · Owns:
  `screens/settings/index.tsx`, `screens/settings/Experience.tsx`, `screens/settings/SyncPrivacy.tsx`,
  `screens/settings/Permissions.tsx`, `screens/settings/Vault.tsx`, `screens/settings/Tools.tsx`,
  `screens/settings/Ai.tsx`, `screens/settings/Account.tsx`, `screens/CommandCenter.tsx`. Gate at
  the section, not only the tab. A section above the active tier is absent from the page, and a deep
  link to it lands on the honest gate with a "Show advanced settings" action (the existing tier
  write). Links elsewhere, such as the Command Center's Manage list today, never point at a surface
  the current tier hides. Changing the tier updates open pages at once. Acceptance: a fixture test
  renders the expected section set per tier; e2e: a Beginner sees no E2EE, permissions or
  diagnostics controls, an Expert sees them all, and switching tiers updates the open page.
- **RC-UX-5.3 — ADR-042: simple modes default to Cloud-Enhanced.** `S` · P2 · Owns:
  `docs/adr/042-tiered-vault-privacy-default.md` (new), `docs/adr/026-opt-in-vault-privacy-modes.md`,
  `docs/adr/README.md`, `docs/security/vault-privacy-modes-threat-model.md`. Amend ADR-026 with the
  2026-09-12 decision (§1.6 D3), which reverses its rejected alternative "silent default to
  Cloud-Enhanced" for the simpler tiers: Beginner and Standard create Cloud-Enhanced vaults with a
  one-line plain disclosure; Private (E2EE) is offered, as an explicit choice, at Expert; the
  sample-or-fresh choice is no longer a forced step. What must still hold: a legacy or absent mode
  reads as Private, so no existing vault changes mode; the disclosure says what the server can read;
  Settings shows the mode and how to change it; the ADR records whether a later move from
  Cloud-Enhanced to Private is supported and what it costs. Acceptance: ADR-042 Accepted; ADR-026
  carries the amended-by link and both index cells match their Status lines; the threat model's
  consent section updated; a `/security-review` report linked in the journal.
  **Done 2026-09-12 — ADR-042 Accepted**, on `loop/rc` and not yet on `main`. Two threads it left open
  belong elsewhere rather than to a re-opened story: its closing line makes server-readable enablement
  depend on "RC-CLD-2.2's separate security review", which is now RC-CLD-6.5; and the rule that a vault
  created under the tiered default gains nothing server-readable without fresh consent is carried by
  RC-CLD-6.3, which amends ADR-042 when it runs.
- **RC-UX-5.4 — Local vaults and the vault switcher.** `L` · P2 · Owns:
  `apps/gm-react/src/platform/storage/coreStore.ts`, `apps/gm-react/src/platform/storage/privateStore.ts`,
  `apps/gm-react/src/runtime/SceneRuntime.ts`, `apps/gm-react/src/runtime/RuntimeContext.tsx`,
  `apps/gm-react/src/cloud/vaultKey.ts`, `apps/gm-react/src/cloud/vaultMode.ts`,
  `apps/gm-react/src/app/shell/VaultSwitcher.tsx` (new), `apps/gm-react/src/app/shell/Sidebar.tsx`.
  Current state: one IndexedDB vault per device. Key storage by vault id so a device holds several
  vaults with nothing shared: core store, private store, keyring, privacy mode, and per-vault
  preferences. The runtime reloads against the chosen vault. The sidebar's campaign chip opens the
  switcher (desktop and rail; the More sheet on phones), which lists vaults with name and last
  opened and lets the GM create, rename and switch, with an entry UX-3.7 fills with the demo. The
  existing vault migrates in place as the first entry. Acceptance: e2e creates a second vault,
  switches, and proves isolation both ways (op counts, search, keyring); a migration test opens a
  pre-switcher vault unchanged; backup and cloud sync act on the active vault only;
  `scene-first-render` in budget.
- **RC-UX-5.5 — Navigation review before release.** `S` · P4 · Deps: CAN-7.9 · Owns:
  `docs/planning/IA_REVIEW.md` (new). Navigation stays unchanged until this review (§1.6 D6). Start
  from the 2026-09-12 findings: 14 primary destinations against the five to seven ADR-013 allows;
  one section under three names (Maps, the atlas route and "Atlas"; Story, the campaign route and
  "Campaign"; Notes, the knowledge route and "Knowledge"); "DM screen" and "GM Screen" on one page;
  Plans & cloud as a primary destination; NPCs reachable from Characters, Story and Notes; two ways
  into the player view (the sidebar footer and the top bar's view-as); four unlabeled top-bar icons
  (two people, two eyes); run screens missing from the phone tab bar; where pinned screens (CAN-7.4)
  fit. Deliver a recommended IA with the alternatives weighed, the route and bookmark migration, and
  the ADR-013 amendment text; implementation is filed as follow-up stories once the recommendation
  is accepted. Acceptance: the review is merged, every finding has a recommendation, and ENG-7.3
  lists the accepted decision as a release precondition.
...
app-security-review-<date>.md`, fixes. Scope: sandbox host (WID-1.3), host API, package review,
  system packages as data, private player store, PWA cache, auto-update signature, Stripe webhook, and
  the closed Cloud-Enhanced gate — that the record ships unapproved and cannot be bypassed, since the
  server-readable path itself is deferred to epic CLD-6 and is reviewed there, in RC-CLD-6.5, not here.
...
- **RC-ENG-7.3 — RC-1 exit audit.** `M` · P4 · Deps: `POL-1.*`, ENG-5.1, DOC-1.1, 8.3, UX-5.5 · Owns:
...
### Epic ENG-8 — Early access: primary paths before platform depth (P2→P3)

The platform (sandboxed widgets, trust review, two builders, the marketplace, System Packages) is
broad, while the 2026-09-12 review found the primary paths shipping an error tile on the seeded home
board, clipped tiles, and several gold primaries on one screen (§1.6 D7). Extensibility continues;
this epic makes the primary paths releasable first and keeps them that way.

- **RC-ENG-8.1 — Golden-path journeys.** `M` · P2 · Owns: `apps/gm-react/tests/e2e/_helpers.ts`,
  `apps/gm-react/tests/e2e/golden-path.spec.ts` (new), `docs/development/TESTING.md`. Four journeys
  on both Playwright profiles: first run to the Command Center on an empty vault; prep (a character,
  a map, a note, a screen from a template, pinned); a session run in Standby and then live (roll,
  table draw, encounter, three rounds with HP and conditions, a handout, map projection, ending with
  capture and recap); and a player joining over the fake LAN transport who sees only player-safe
  content. Each journey asserts no console error or warning, no failed request, no widget in an
  error state, no disabled control without a stated reason, and no unrecoverable clip, using an
  overflow detector in the e2e helpers that flags content overflowing without a scroll affordance or
  an accessible full text. Acceptance: the spec runs in the manifest's browser gate; each assertion
  fails on a seeded defect (tests); TESTING.md documents the journeys and the detector.
- **RC-ENG-8.2 — Default content never shows an error.** `S` · P2 · Owns:
  `packages/core/src/commands/command-center.ts`, `apps/gm-react/src/runtime/demo-seed.ts`. Current
  state: the seeded home board's Map tile opens on "The linked map is missing or was removed." Fix
  the seed's binding, and add a test that resolves every widget binding on every default screen and
  in the demo seed to available (or to an intentional empty state in an empty vault), never missing,
  conflicted or degraded. Acceptance: the test; the ENG-8.1 journeys see no error tile.
- **RC-ENG-8.3 — Early-access exit audit.** `M` · P3 · Deps: CAN-7.8, CAN-7.4, CAN-5.4, CAN-5.5,
  CAN-6.4, SES-6.2, UX-3.6, UX-3.7, UX-5.2, 8.4, POL-1.1, POL-1.2, POL-1.4, POL-1.5, POL-1.8,
  POL-1.9, POL-1.11, POL-1.18, POL-1.23 · Owns: `docs/planning/EARLY_ACCESS.md` (new). The M2.5
  gate: golden paths green on five consecutive runs on both profiles; a `ux-ui-reviewer` pass over
  every primary route at the three tiers with zero high findings; the a11y gate clean; no open
  primary-path entry in DEBT.md; no primary FEATURE-GAPS row carrying a limit beyond the §2.1 four.
  Findings become stories that block this one. Acceptance: EARLY_ACCESS.md records the run, the
  review and the verdict on one commit.
- **RC-ENG-8.4 — Emphasis and display-face lint.** `S` · P2 · Owns: `scripts/emphasis-lint.ts`
  (new), `scripts/emphasis-baseline.json` (new). Two design rules the screens break today become
  checks in `pnpm lint`: the display face (Cinzel) set below 24px, and more than one accent-filled
  primary inside one region (Session in Standby shows three: the Standby segment, Go live and Build
  encounter). The lint reads inline styles and DS props across the app, starts as a warning against
  a counted baseline that may only shrink, and names the offending file and line. The fixes land in
  CAN-5.5, SES-6.2 and the POL stories, none of which may raise the count. Acceptance: a fixture
  test per rule; the baseline committed; the lint wired into `pnpm lint`.
...
- **RC-POL-1.1 — Polish: Command Center.** `M` · P3 · Deps: DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, UX-3.7, CAN-7.6 · Owns: `screens/CommandCenter.tsx`, `app/widgets/templates`, `screens/SceneCardsPanel.tsx`, `screens/SceneQueuePanel.tsx`, `screens/ScenesCreator.tsx`. One
...
- **RC-POL-1.2 — Polish: GM Screen board.** `L` · P3 · Deps: DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, CAN-2.5, CAN-3.6, CAN-4.6, CAN-5.2, CAN-5.4, CAN-5.5, CAN-6.3, CAN-6.4, CAN-7.3, CAN-7.7 · Owns: `screens/Board.tsx`, `apps/gm-react/src/screens/screen`, `screens/board`, `screens/BoardLayoutsPanel.tsx`, `app/SceneBoardCanvas.tsx`, `app/canvas`, `app/board-helpers.ts`, `app/SceneBoardModel.ts`. One
  change that walks §20.2–§20.5 for this surface: every item checked in the run journal or waived with a
  reason a reviewer would accept; visual snapshots (RC-DSN-4.1) updated for every theme and tier; empty,
  loading and error states illustrated where the key exists; copy re-read in the voice with ES updated;
  the FEATURE-GAPS row edited; every file in the surface under 500 lines. Acceptance: the journal embeds
...
- **RC-POL-1.3 — Polish: Scene editor.** `M` · P3 · Deps: DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, CAN-4.2, CAN-4.4, CAN-4.6, CAN-6.1, WID-2.6, WID-4.3 · Owns: `screens/sceneEditor`. One
  ...
  the checklist; axe clean for the route and its overlays on both profiles; the surface's e2e specs
  green; `pnpm gates` reports no file-size warning for the owned files.
- **RC-POL-1.4 — Polish: Session.** `L` · P3 · Deps: DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, SES-2.4, SES-5.1, SES-6.2, CAN-7.8 · Owns: `screens/session`, `app/session`, `app/combat`, `app/EncounterBuilder.tsx`, `app/EncounterDraft.ts`, `app/EncounterDraftRoster.tsx`. One
  change that walks §20.2–§20.5 for this surface: every item checked in the run journal or waived with a
  reason a reviewer would accept; visual snapshots (RC-DSN-4.1) updated for every theme and tier; empty,
  loading and error states illustrated where the key exists; copy re-read in the voice with ES updated;
  the FEATURE-GAPS row edited; every file in the surface under 500 lines. Acceptance: the journal embeds
...
- **RC-POL-1.16 — Polish: Plans, cloud and legal.** `M` · P3 · Deps: DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, CLD-2.8 · Owns: `screens/Upgrade.tsx`, `screens/upgrade`, `screens/legal`. One
  ...
  the checklist; axe clean for the route and its overlays on both profiles; the surface's e2e specs
  green; `pnpm gates` reports no file-size warning for the owned files.
- **RC-POL-1.17 — Polish: Settings (every category).** `L` · P3 · Deps: DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, KNW-1.4, UX-1.5, CLD-2.7, UX-5.2 · Owns: `screens/settings`, `app/ConnectedSources.tsx`, `app/connectedSourcesVocab.ts`. One
  change that walks §20.2–§20.5 for this surface: every item checked in the run journal or waived with a
  reason a reviewer would accept; visual snapshots (RC-DSN-4.1) updated for every theme and tier; empty,
  loading and error states illustrated where the key exists; copy re-read in the voice with ES updated;
  the FEATURE-GAPS row edited; every file in the surface under 500 lines. Acceptance: the journal embeds
...
- **RC-POL-1.22 — Polish: Command palette, shortcuts and help.** `M` · P3 · Deps: DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, CAN-4.3 · Owns: `app/CommandPalette.tsx`, `app/shortcuts`, `app/help`. One
  ...
  the checklist; axe clean for the route and its overlays on both profiles; the surface's e2e specs
  green; `pnpm gates` reports no file-size warning for the owned files.
- **RC-POL-1.23 — Polish: App shell (sidebar, rail, tabs, top bar, footer).** `M` · P3 · Deps: DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, UX-4.2, UX-4.3, CAN-5.2, CAN-7.4, UX-5.4 · Owns: `app/AppShell.tsx`, `app/shell`, `app/nav.ts`, `app/useViewport.ts`. One
  change that walks §20.2–§20.5 for this surface: every item checked in the run journal or waived with a
  reason a reviewer would accept; visual snapshots (RC-DSN-4.1) updated for every theme and tier; empty,
  loading and error states illustrated where the key exists; copy re-read in the voice with ES updated;
  the FEATURE-GAPS row edited; every file in the surface under 500 lines. Acceptance: the journal embeds
...
| External blockers (Stripe, certs, Firebase, curation) slip                       | High       | Medium | Those stories are isolated behind fail-closed gates; RC-1 ships with honest "not in this edition" only for these and the deferred Cloud-Enhanced phase 2 (epic CLD-6, a scope decision, §2.1).                                            |
...
| Converting the Command Center to widgets loses polish or behaviour               | Medium     | High   | CAN-7.5 parity matrix and committed baselines; a before-and-after `ux-ui-reviewer` pass; the WID-5.5 builder parity gate; bespoke screens retired only after a promotion (CAN-7.9).                                                       |
| Per-vault storage (UX-5.4) mixes or loses data                                   | Medium     | High   | Isolation e2e both ways; an in-place migration test on a pre-switcher vault; backup and sync scoped to the active vault.                                                                                                                  |
| Defaulting simple tiers to Cloud-Enhanced weakens privacy expectations           | Medium     | High   | ADR-042 with a security review; a legacy or absent mode stays Private; a plain disclosure; Private one tier switch away; while the gate is closed the mode is E2EE in fact, and opening it later needs fresh per-vault consent (CLD-6.3). |
| The Screens chain queues behind the CAN lane and delays early access             | High       | Medium | SES-6, UX-5, CAN-6.4, CAN-7.1/7.5, WID-5.1/5.2/5.4 and ENG-8 touch no canvas files and run now; CAN-7 depends only on the CAN stories whose files it shares.                                                                              |
...
_330 stories. By size: L=35 · M=176 · S=119. By phase: P0=20 · P1=28 · P2=188 · P3=74 · P4=19 · rolling=1. Status (from the dispatcher store): blocked 28 · done 197 · in progress 3 · open 94 · operator 7 · skipped 1. Rendered by `tools/roadmap/sync-status.py`; the store, not this column, is the record._
...
| RC-STB-1.4  | Stabilize   | Branch ledger and dispatcher worktree hygiene                                                                                                                                                                                                                                                              | S    | P0      | 1.2                                                                                                                                                        | done (3843952)       |
...
| RC-WID-2.4  | Widgets     | Style step                                                                                                                                                                                                                                                                                                 | S    | P2      | 2.1, DSN-1.1                                                                                                                                               | done (f543ec9)       |
...
| RC-CAN-2.3  | Canvas      | Note tile depth levels                                                                                                                                                                                                                                                                                     | M    | P2      | 2.2, KNW-1.1                                                                                                                                               | blocked(review)      |
| RC-CAN-2.4  | Canvas      | Tile action menu                                                                                                                                                                                                                                                                                           | M    | P2      | 2.2, CAN-1.3                                                                                                                                               | done (6a79451)       |
| RC-CAN-2.5  | Canvas      | Resize presets and keyboard resize                                                                                                                                                                                                                                                                         | S    | P2      | 2.4                                                                                                                                                        | blocked(review)      |
...
| RC-CAN-3.5  | Canvas      | Keyboard model completion                                                                                                                                                                                                                                                                                  | M    | P2      | 1.3, 2.4                                                                                                                                                   | blocked(write fence) |
| RC-CAN-3.6  | Canvas      | Multi-select, align/distribute, group, z-order                                                                                                                                                                                                                                                             | L    | P2      | 1.1, 3.5                                                                                                                                                   |                      |
| RC-CAN-4.1  | Canvas      | Tile gallery sheet with live previews                                                                                                                                                                                                                                                                      | M    | P2      | 2.2, WID-1.1                                                                                                                                               | blocked(rebase)      |
...
| RC-CAN-5.1  | Canvas      | Compact stacked-panel board                                                                                                                                                                                                                                                                                | L    | P2      | 2.2                                                                                                                                                        | blocked(review)      |
...
| RC-CAN-6.1  | Canvas      | Player-view preview overlay on the canvas                                                                                                                                                                                                                                                                  | M    | P2      | 2.2                                                                                                                                                        | blocked(review)      |
...
| RC-MAP-4.5  | Maps        | Editor onboarding and shortcut discovery                                                                                                                                                                                                                                                                   | S    | P3      | UX-3.3                                                                                                                                                     | blocked(review)      |
...
| RC-SES-2.4  | Session     | Dice drama                                                                                                                                                                                                                                                                                                 | S    | P3      | DSN-1.3                                                                                                                                                    | done (b54cf4c)       |
...
| RC-CHR-2.4  | Characters  | Printable sheet                                                                                                                                                                                                                                                                                            | S    | P3      | —                                                                                                                                                          | done (338e06c)       |
...
| RC-CHR-5.2  | Characters  | Builder step polish                                                                                                                                                                                                                                                                                        | S    | P3      | —                                                                                                                                                          | blocked(review)      |
| RC-CHR-5.3  | Characters  | Roster library information scent                                                                                                                                                                                                                                                                           | S    | P3      | —                                                                                                                                                          | done (d16482c)       |
...
| RC-KNW-1.4  | Knowledge   | Reading width and typography preference                                                                                                                                                                                                                                                                    | S    | P3      | —                                                                                                                                                          | blocked(review)      |
| RC-KNW-2.1  | Knowledge   | Filters and saved searches UI                                                                                                                                                                                                                                                                              | M    | P2      | —                                                                                                                                                          | done (a61091b)       |
| RC-KNW-2.2  | Knowledge   | Note list information scent                                                                                                                                                                                                                                                                                | S    | P3      | —                                                                                                                                                          | blocked(review)      |
...
| RC-KNW-4.3  | Knowledge   | Graph performance and interaction                                                                                                                                                                                                                                                                          | S    | P3      | —                                                                                                                                                          | done (128227a)       |
...
| RC-AUD-2.4  | Audio       | Second-screen display v2                                                                                                                                                                                                                                                                                   | S    | P3      | —                                                                                                                                                          | done (ac99ca7)       |
...
| RC-AI-4.1   | AI          | Copilot client + indexer contract (behind the phase-2 gate)                                                                                                                                                                                                                                                | M    | P2      | CLD-2.2 (switch-on: CLD-6.5)                                                                                                                               | done                 |
...
| RC-CLD-2.2  | Cloud       | Cloud-Enhanced gate closure and honest limit                                                                                                                                                                                                                                                               | S    | P2      | —                                                                                                                                                          |                      |
...
| RC-CLD-4.4  | Cloud       | Wiki v2                                                                                                                                                                                                                                                                                                    | M    | P3      | —                                                                                                                                                          | blocked(review)      |
| RC-CLD-4.5  | Cloud       | Discovery: search, filters, featured, ratings                                                                                                                                                                                                                                                              | M    | P2      | 4.1, 4.3                                                                                                                                                   | blocked(blocked)     |
| RC-CLD-6.1  | Cloud       | Server-side authority for the vault mode                                                                                                                                                                                                                                                                   | L    | P4      | —                                                                                                                                                          | blocked(deferred)    |
| RC-CLD-6.2  | Cloud       | The gated server-readable content path                                                                                                                                                                                                                                                                     | L    | P4      | 6.1                                                                                                                                                        | blocked(deferred)    |
| RC-CLD-6.3  | Cloud       | Mode transitions, deletion, and the re-consent rule                                                                                                                                                                                                                                                        | L    | P4      | 6.2                                                                                                                                                        | blocked(deferred)    |
| RC-CLD-6.4  | Cloud       | Features on the consented boundary                                                                                                                                                                                                                                                                         | L    | P4      | 6.3                                                                                                                                                        | blocked(deferred)    |
| RC-CLD-6.5  | Cloud       | Phase-2 sign-off and the gate flip                                                                                                                                                                                                                                                                         | M    | P4      | 6.1–6.4                                                                                                                                                    | blocked(deferred)    |
| RC-DSN-1.1  | Design      | Complete the `T` map and lint raw values                                                                                                                                                                                                                                                                   | M    | P3      | —                                                                                                                                                          | done (d4f2e5a)       |
| RC-DSN-1.2  | Design      | Five themes                                                                                                                                                                                                                                                                                                | M    | P3      | —                                                                                                                                                          | blocked(write fence) |
| RC-DSN-1.3  | Design      | Motion vocabulary                                                                                                                                                                                                                                                                                          | S    | P3      | —                                                                                                                                                          | done (6f80c08)       |
| RC-DSN-1.4  | Design      | Density audit                                                                                                                                                                                                                                                                                              | S    | P3      | —                                                                                                                                                          | blocked(review)      |
| RC-DSN-2.1  | Design      | Convert `src/ds/components/                                                                                                                                                                                                                                                                                | L    | P3      | —                                                                                                                                                          |                      |
| RC-DSN-2.2  | Design      | Missing primitives                                                                                                                                                                                                                                                                                         | M    | P3      | —                                                                                                                                                          | done (2112968)       |
| RC-DSN-2.3  | Design      | Component documentation site (in-repo)                                                                                                                                                                                                                                                                     | M    | P3      | —                                                                                                                                                          | done (ba6b3d9)       |
| RC-DSN-2.4  | Design      | Re-sync design source A                                                                                                                                                                                                                                                                                    | S    | P3      | —                                                                                                                                                          |                      |
| RC-DSN-3.1  | Design      | Empty-state illustration set                                                                                                                                                                                                                                                                               | M    | P3      | —                                                                                                                                                          | blocked(write fence) |
| RC-DSN-3.2  | Design      | Icon vocabulary completion                                                                                                                                                                                                                                                                                 | S    | P3      | —                                                                                                                                                          | blocked(blocked)     |
| RC-DSN-3.3  | Design      | Brand asset kit                                                                                                                                                                                                                                                                                            | M    | P3      | —                                                                                                                                                          | blocked(blocked)     |
| RC-DSN-3.4  | Design      | Loading, skeleton, and progress states                                                                                                                                                                                                                                                                     | S    | P3      | —                                                                                                                                                          | blocked(blocked)     |
| RC-DSN-4.1  | Design      | Golden-route visual regression suite                                                                                                                                                                                                                                                                       | M    | P3      | —                                                                                                                                                          | blocked(review)      |
...
| RC-UX-2.3   | UX          | Focus and dialog audit                                                                                                                                                                                                                                                                                     | S    | P3      | —                                                                                                                                                          | blocked(review)      |
| RC-UX-2.4   | UX          | Text scaling and zoom                                                                                                                                                                                                                                                                                      | S    | P3      | —                                                                                                                                                          | in progress          |
...
| RC-UX-3.6   | UX          | Onboarding v3: short, tiered and friendly                                                                                                                                                                                                                                                                  | M    | P2      | 5.2, 5.3                                                                                                                                                   |                      |
| RC-UX-3.7   | UX          | The demo campaign as inspiration, from the vault switcher                                                                                                                                                                                                                                                  | M    | P2      | 5.4                                                                                                                                                        |                      |
| RC-UX-4.1   | UX          | Device-preferences slice and platform layer (DEBT-2026-001)                                                                                                                                                                                                                                                | M    | P2      | —                                                                                                                                                          | done (c8056a7)       |
| RC-UX-4.2   | UX          | Mobile primary-action audit                                                                                                                                                                                                                                                                                | S    | P3      | —                                                                                                                                                          | in progress          |
| RC-UX-4.3   | UX          | Tablet (rail) layouts                                                                                                                                                                                                                                                                                      | M    | P3      | —                                                                                                                                                          |                      |
| RC-UX-4.4   | UX          | Copy pass v2 in the Lamplight voice                                                                                                                                                                                                                                                                        | M    | P3      | 1.2                                                                                                                                                        | in progress          |
...
| RC-PLT-1.3  | Platform    | Electron parity audit                                                                                                                                                                                                                                                                                      | S    | P3      | —                                                                                                                                                          | blocked(review)      |
...
| RC-ENG-2.3  | Engineering | A merge gate the autonomous loop cannot pass blind                                                                                                                                                                                                                                                         | M    | P2      | —                                                                                                                                                          | done (1111f0f)       |
...
| RC-ENG-2.6  | Engineering | Mobile-chromium navigation flake root cause                                                                                                                                                                                                                                                                | S    | P2      | —                                                                                                                                                          | done (958d842)       |
...
| RC-ENG-4.4  | Engineering | Dependency wave (the 2026-09 dependabot set)                                                                                                                                                                                                                                                               | M    | P2      | —                                                                                                                                                          | blocked(blocked)     |
...
| RC-ENG-7.3  | Engineering | RC-1 exit audit                                                                                                                                                                                                                                                                                            | M    | P4      | `POL-1.*`, ENG-5.1, DOC-1.1, 8.3, UX-5.5                                                                                                                   |                      |
...
| RC-DOC-2.2  | Docs        | Docs link and coupling checker in the gates                                                                                                                                                                                                                                                                | S    | P2      | —                                                                                                                                                          | blocked(blocked)     |
| RC-POL-1.1  | Polish      | Polish: Command Center                                                                                                                                                                                                                                                                                     | M    | P3      | DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, UX-3.7, CAN-7.6                                                                                               |                      |
| RC-POL-1.2  | Polish      | Polish: GM Screen board                                                                                                                                                                                                                                                                                    | L    | P3      | DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, CAN-2.5, CAN-3.6, CAN-4.6, CAN-5.2, CAN-5.4, CAN-5.5, CAN-6.3, CAN-6.4, CAN-7.3, CAN-7.7                      |                      |
...
| RC-POL-1.4  | Polish      | Polish: Session                                                                                                                                                                                                                                                                                            | L    | P3      | DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, SES-2.4, SES-5.1, SES-6.2, CAN-7.8                                                                            |                      |
...
| RC-POL-1.17 | Polish      | Polish: Settings (every category)                                                                                                                                                                                                                                                                          | L    | P3      | DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, KNW-1.4, UX-1.5, CLD-2.7, UX-5.2                                                                              |                      |
...
| RC-POL-1.23 | Polish      | Polish: App shell (sidebar, rail, tabs, top bar, footer)                                                                                                                                                                                                                                                   | M    | P3      | DSN-1.2, DSN-1.3, DSN-3.1, DSN-3.4, DSN-4.1, UX-4.2, UX-4.3, CAN-5.2, CAN-7.4, UX-5.4                                                                      |                      |

---

| RC-WID-5.1 | Widgets | Navigation and creation intents | M | P2 | CAN-7.5 | |
| RC-WID-5.2 | Widgets | Query sources for hub screens | M | P2 | 5.1 | |
| RC-WID-5.3 | Widgets | Hub template renderers and bare presentation | M | P2 | 5.2 | |
| RC-WID-5.4 | Widgets | The design-system kit inside the sandbox | M | P2 | 2.4 | |
| RC-WID-5.5 | Widgets | Builder parity gate | S | P2 | 5.3, CAN-7.6 | |
| RC-CAN-5.4 | Canvas | Moving around a canvas screen on a phone | M | P2 | 5.1 | |
| RC-CAN-5.5 | Canvas | Tile content never clips past recovery | M | P2 | ENG-8.1 | |
| RC-CAN-6.4 | Canvas | Visibility badges by exception | S | P2 | none | |
| RC-CAN-7.1 | Canvas | ADR-041: Screens | S | P2 | none | |
| RC-CAN-7.2 | Canvas | Screen metadata, pins and the screens read | M | P2 | 7.1 | |
| RC-CAN-7.3 | Canvas | The screens library and switcher | L | P2 | 7.2, 4.4, 5.1 | |
| RC-CAN-7.4 | Canvas | Pinned screens in the sidebar, rail and More sheet | M | P2 | 7.3 | |
| RC-CAN-7.5 | Canvas | Parity inventory of the three run surfaces | S | P2 | 7.1 | |
| RC-CAN-7.6 | Canvas | The Command Center as the default screen | L | P2 | 7.3, 7.5, 7.7, WID-5.1, WID-5.2, WID-5.3, ENG-8.2 | |
| RC-CAN-7.7 | Canvas | The flow layout policy | L | P2 | 7.2 | |
| RC-CAN-7.8 | Canvas | Session as a screen | L | P2 | 7.6, SES-6.2, WID-5.5 | |
| RC-CAN-7.9 | Canvas | Retire the bespoke run screens | S | P3 | 7.4, 7.6, 7.8 | |
| RC-SES-6.1 | Session | Only the clock, automations and start triggers wait for Go live | M | P2 | none | |
| RC-SES-6.2 | Session | Remove the Go-live gates from the UI | M | P2 | 6.1 | |
| RC-UX-5.1 | UX | The feature complexity map | M | P2 | none | |
| RC-UX-5.2 | UX | Tiers decide what Settings shows | M | P2 | 5.1 | |
| RC-UX-5.3 | UX | ADR-042: simple modes default to Cloud-Enhanced | S | P2 | none | |
| RC-UX-5.4 | UX | Local vaults and the vault switcher | L | P2 | none | |
| RC-UX-5.5 | UX | Navigation review before release | S | P4 | CAN-7.9 | |
| RC-ENG-8.1 | Engineering | Golden-path journeys | M | P2 | none | |
| RC-ENG-8.2 | Engineering | Default content never shows an error | S | P2 | none | |
| RC-ENG-8.3 | Engineering | Early-access exit audit | M | P3 | CAN-7.8, CAN-7.4, CAN-5.4, CAN-5.5, CAN-6.4, SES-6.2, UX-3.6, UX-3.7, UX-5.2, 8.4, POL-1.1, POL-1.2, POL-1.4, POL-1.5, POL-1.8, POL-1.9, POL-1.11, POL-1.18, POL-1.23 | |
| RC-ENG-8.4 | Engineering | Emphasis and display-face lint | S | P2 | none | |
