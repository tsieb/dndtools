...
- It DECLARES its bundle/memory budgets in the PERF-001 registry shape and MEASURES them through the
  ...
  `measurement.ts`). There is exactly one measurement API in the codebase; every new size/footprint/
...
- It COMPOSES the MCP-001 default-off optionality (`state/mcp-policy.ts` `isMcpEnabled` /
  `EMPTY_MCP_POLICY_STATE`) and the AI-boundary contract (`mcp/ai-boundary.ts`): the AI/MCP isolation
  proof grades a core measurement under MCP-enabled vs MCP-disabled and across every AI capability
  state, proving the core path is INDEPENDENT of AI/MCP.
- It COMPOSES the diagnostics REDACTION guard (`diagnostics/redaction.ts` `redactValue` /
  `containsSensitiveData`) and the SEC-010 stream-privacy needle scanner (`collab/stream-privacy.ts`
  `findStreamPrivacyLeaks`) for the privacy-preserving diagnostics — the same scrubber/scanner the
  support-bundle, content-export, and replication-stream paths already use.
...
- Playwright e2e (`pnpm e2e`, desktop-chromium + mobile-chromium) — NOT RUN, justified: this epic is
  ...
  (a PRECISION fix to the existing boundary test so a relative `../mcp/` intra-core import is judged by
  resolving its target inside `apps/v2` rather than by substring — strengthening, not weakening, the
...
- `apps/v2/packages/core/tests/boundary.test.ts` (modified) — precise relative-import resolution so a
  legitimate intra-core `../mcp/` import is allowed while a genuine v1 `mcp/` import (which escapes
  `apps/v2`) is still forbidden.
...
- Commits:
  - feature + tests + completion evidence: `__FEATURE_SHA__` (`feat(v2): complete
    PERF-bundles-memory-and-ai-mcp-isolation epic`).
  - regenerated derived planning files after `v2:workpack:complete`: `__COMPLETE_SHA__`.
  - this SHA-record follow-up: `__RECORD_SHA__` (the HEAD of this branch).

Workpack status: `complete`

Final `git status --short` (clean tree at handoff after all commits):
