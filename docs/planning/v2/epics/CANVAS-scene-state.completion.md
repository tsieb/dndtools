...
- **Contract 1 (Processing/Display Decoupling)** — `@dndtools/v2-core` has no Svelte/DOM/Node/v1
  imports (enforced by `scripts/v2-boundary-lint.ts` +
  `apps/v2/packages/core/tests/boundary.test.ts`). All durable
...
This completion evidence predates the programmatic git-slate rule. The completed epic now resolves
through `docs/planning/v2/workpack-state.yaml`, and future completion files must paste the exact
