...
| **R** | **This repo** (`apps/gm`, `apps/gm-react`, `packages/core`) | Production code | The shipping product. `apps/gm` is SvelteKit 5; `apps/gm-react` is the React port. Reskinned _toward_ A/B. | Claude Code directly. |
...
│
└── handoff/ runbook ──►  R  (apps/gm + apps/gm-react: redesign.tokens.css + APPLY.md
...
| The retired pre-prototype screen mocks | `docs/design/_archive/command-center-kit/` (see below) |
...
On 2026-06-23 the kit was **trimmed from A** so that A owns parts and B owns the assembled app. To
keep the trim non-destructive, the **entire kit source was archived first** into
[`_archive/command-center-kit/`](./_archive/command-center-kit/) (git-recoverable). A's `readme.md`,
`SKILL.md`, and `handoff/APPLY.md` were repointed from the kit to B + the design-system templates.
...
2. **A→repo handoff.** Keep `handoff/APPLY.md` + `handoff/redesign.tokens.css` current as A evolves;
   they are the bridge into `apps/gm`.
