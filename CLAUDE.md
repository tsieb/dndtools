...
Architecture Decisions: `docs/adr/README.md` Ã¢â‚¬â€ ADR index (ADR-001 through ADR-013)
...
- **I21 — Codebase Realignment & Quality Audit**: In Progress. Phase A complete (CI restructuring, build script audit, metrics baseline — Epics 21.1–21.3). Phase B advanced: Epic 21.4 (project organization & goal alignment audit) shipped — compliance matrix, route/IA audit, file structure audit, feature tier audit, exit criteria verification for I1–I20, and known gaps reconciliation documented in `docs/audits/21.4-project-organization-goal-alignment.md` with prioritized remediation backlog. Epic 21.5 (module boundary & dependency audit) shipped — renderer/storage boundary clean (ESLint enforced), Electron isolation verified (context isolation + sandbox + 42 IPC schemas), MCP filesystem boundary 98% clean with `scripts/mcp-boundary-lint.ts` enforcement, zero circular dependencies with madge integration, 68 dependencies audited (no unused), type export boundary violations documented with remediation plan, plus repo boundary/cycle audits, coverage enforcement in CI, and browser markdown archive export validation. Full report: `docs/audits/21.5-module-boundary-dependency-audit.md`. Epic 21.6 (ADR & standards compliance review) shipped — all 10 existing ADRs verified current, 3 new ADRs drafted (ADR-011 theme presets, ADR-012 progressive disclosure, ADR-013 navigation contract), coding standards scan clean (1 acceptable `as any`), design token lint and navigation lint both zero violations. Full report: `docs/audits/21.6-adr-standards-compliance-review.md`. Remaining: Epics 21.7–21.8 (file decomposition, bundle optimization).
