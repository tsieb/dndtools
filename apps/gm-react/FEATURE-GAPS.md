...
**Honest stubs (no Core backing — labeled in-UI):** Extensions install/enable, Community publish/wiki/discover, billing/account/subscription, AI-provider config, the map **pixel renderer** (ADR-014 — data/control layer is wired, painting is a stylized placeholder), Campaign **factions** (no Core entity).
