...
test.describe('widget builder: style step (RC-WID-2.4)', () => {
	...
	for (const custom of [false, true]) {
		test(`declares a semantic token that the Inspector lists and that re-themes with data-theme (${custom ? 'iframe' : 'template'})`, async ({
			page,
		}) => {
			const dialog = await openBuilder(page);
			await dialog.getByLabel('Name', { exact: true }).fill('Party status');
			...
			expect((await underTheme('tavern')).frame).toBe(tavern.frame);
		});
	}
});
