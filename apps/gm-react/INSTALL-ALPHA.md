...
These alpha builds are **not code-signed**, so macOS and Linux will show a one-time security prompt the
first time you open the app. That's expected for an unsigned alpha; the steps below get you past it.
...
- **Windows isn't built yet** — only macOS and Linux for this alpha.
