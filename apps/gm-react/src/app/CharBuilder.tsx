...
const KIND_LABEL: Record<CharKind, string> = {
	pc: 'PC',
	npc: 'NPC',
	monster: 'Monster',
	sidekick: 'Sidekick',
};
const KIND_TONE: Record<CharKind, string> = {
	pc: 'success',
	npc: 'info',
	monster: 'error',
	sidekick: 'warning',
					...
					<div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '14px 28px 20px' }}>
						{step.id === 'identity' && (
							<div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
									...
									<div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 16 }}>
										<div>
											...
											{players.length > 0 ? (
												...
												<HonestNote>
													Add a player in Settings before building a player character.
												</HonestNote>
											)}
										</div>
									</div>
								...
								<div>
									<FieldLabel hint="Sets racial traits & ability bonuses">
										Ancestry / race
									</FieldLabel>
									<div
										style={{
									...
									{BUILDER.abilityKeys.map((k) => (
										...
										>
											...
											{method === 'standard' ? (
												...
												<>
													<span style={{ font: `700 22px ${T.mono}`, color: T.ink }}>
														{scores[k]}
													</span>
						...
						{step.id === 'kit' && (
							<div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
								...
								<div>
									<FieldLabel hint="Attacks, cantrips, and signature moves">
										Attacks &amp; actions
									</FieldLabel>
						...
						{step.id === 'bio' && (
							<div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
								...
								<div>
									...
									{isPc ? (
										<HonestNote>
											A new PC starts <strong>shared with its owning player</strong> — the core's
											guided-flow rule (CHAR-002). The DM can widen who sees it afterwards from the
											character sheet's Sharing controls.
										</HonestNote>
									) : (
					...
					>
						...
						>
							...
							<div style={{ font: `12.5px/1.6 ${T.sans}`, color: T.sub }}>
								The wizard hasn't created anything yet — closing now throws away everything entered
								so far.
							</div>
