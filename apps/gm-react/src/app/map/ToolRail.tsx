...
export function ToolRail({
	...
	// column, so an always-open flyout is a permanent opaque ~190px panel sitting on the map's
	// top-left corner with nothing under the DM's control to move it. Collapsing is now a real path:
	// pressing the group you are already showing hides it, which is also what makes `aria-expanded`
	// honest (it used to be omitted precisely because there was no close path).
	const [collapsed, setCollapsed] = useState(false);
	// Keep the flyout following the active tool (e.g. after a keyboard shortcut switches tools) —
	// but do not re-open a flyout the DM deliberately put away.
		...
		>
			...
			>
				{TOOL_GROUPS.map((g) => {
					...
					const isOpen = g.id === openGroup && !collapsed;
					return (
						<button
							key={g.id}
							type="button"
							...
							onClick={() => {
								// Pressing the group whose flyout is already showing puts it away. That
								// close path is what makes `aria-expanded` above honest — it used to be
								// deliberately omitted because the handler could only ever OPEN.
