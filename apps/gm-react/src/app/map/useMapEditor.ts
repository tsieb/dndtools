...
import type { CoreCommand, MapLayerCategory } from '@dndtools/core';
import {
	buildMapInverse,
	...
	const run = useCallback(
		async (command: CoreCommand, opts?: { undoable?: boolean }): Promise<boolean> => {
			...
			try {
				...
				setNotice(error instanceof Error ? error.message : t('atlas.actionFailed'));
				return false;
			} finally {
		...
		[runtime, writeHistory, writeRedo, setNotice, t],
	);
