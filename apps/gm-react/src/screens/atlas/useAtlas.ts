...
export function useAtlas() {
	...
	useEffect(() => {
		...
		if (linkMap !== null) {
			if (maps.some((mp) => mp.id === linkMap)) {
				...
				fail(t('atlas.linkUnavailable'));
			}
		} else if (linkPoi !== null) {
			setSelPoiId(linkPoi);
		}
		setSearchParams({}, { replace: true });
	}, [loading, searchParams, maps, setSearchParams, t]);
	...
	const run = async (command: Parameters<typeof runtime.dispatch>[0]) => {
		...
		try {
			...
			fail(error instanceof Error ? error.message : t('atlas.actionFailed'));
			return undefined;
		} finally {
			setBusy(false);
		}
	...
	function addLayer() {
		...
		void run({
			...
			payload: {
				mapId: selectedId,
				name: t('atlas.layerName', { count: layers.length + 1 }),
	...
	async function deletePoi(poiId: string) {
		...
		Toaster.success(t('atlas.poiDeleted', { name: prior.label }), {
				...
				void runtime
					...
					.then((restored) => {
						if (restored.status === 'accepted')
							Toaster.success(t('atlas.poiRestored', { name: prior.label }));
						else Toaster.error(restored.rejection.message ?? t('atlas.restoreFailed'));
					})
					.catch(() => Toaster.error(t('atlas.restoreFailed')));
			},
		});
	}
	...
	async function copyPoiLink(poiId: string) {
		...
		if (await copyToClipboard(url)) {
			say(t('atlas.linkCopied'));
		} else {
			fail(t('atlas.linkCopyFailed', { url }));
		}
	}
	...
	async function projectToPlayers() {
		...
		if (projected.status === 'accepted') {
			say(t('atlas.projected', { name: selectedEntry?.name ?? '', count: players.length }));
		} else {
