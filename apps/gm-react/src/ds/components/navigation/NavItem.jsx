 ...
 */
export function NavItem({ icon, label, active = false, collapsed = false, badge, onClick, as = 'button', href, style, ...rest }) {
	...
	return (
		<Tag
			...
			onMouseEnter={(e) => { if (!active) { e.currentTarget.style.background = 'var(--color-interactive-hover)'; e.currentTarget.style.color = 'var(--color-text-primary)'; } }}
			...
			{...rest}
		>
			<Icon name={icon} size="sm" label={collapsed ? undefined : null} aria-hidden={!collapsed} style={{ flex: '0 0 auto' }} />
			{!collapsed && <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{label}</span>}
			{!collapsed && badge != null && (
				<span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-2xs)', fontWeight: 'var(--font-weight-bold)', padding: '1px var(--space-1-5)', borderRadius: 'var(--radius-full)', background: active ? 'var(--color-accent)' : 'var(--color-surface-overlay)', color: active ? 'var(--color-accent-foreground)' : 'var(--color-text-secondary)' }}>{badge}</span>
			)}
			{collapsed && badge != null && (
				<span aria-hidden="true" style={{ position: 'absolute', marginTop: -16, marginLeft: 16, width: 7, height: 7, borderRadius: 'var(--radius-full)', background: 'var(--color-accent)', border: '1.5px solid var(--color-surface)' }} />
			)}
		</Tag>
	);
}
