...
export function DataTable({ columns = [], rows = [], rowKey, sort, onSort, zebra = true, dense = false, empty, style, ...rest }) {
	...
	return (
		...
		<table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-sans)', ...style }} {...rest}>
						...
						return (
							...
							>
								<span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexDirection: c.align === 'right' ? 'row-reverse' : 'row' }}>
									{c.header}
									{c.sortable && <Icon name={active ? (sort.dir === 'desc' ? 'chevron-down' : 'chevron-up') : 'chevron-down'} size={12} style={{ opacity: active ? 1 : 0.35 }} />}
								</span>
							</th>
						);
					})}
			...
			<tbody>
				{rows.length === 0 && (
					<tr><td colSpan={columns.length} style={{ padding: 'var(--space-6)', textAlign: 'center', color: 'var(--color-text-tertiary)', fontSize: 'var(--text-sm)' }}>{empty || 'Nothing here yet.'}</td></tr>
				)}
				{rows.map((row, i) => (
					<tr
						key={rowKey ? rowKey(row, i) : i}
						style={{ background: zebra && i % 2 === 1 ? 'var(--color-surface-alt)' : 'transparent', transition: 'background var(--duration-fast) var(--easing-standard)' }}
						onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--color-interactive-hover)'; }}
						onMouseLeave={(e) => { e.currentTarget.style.background = zebra && i % 2 === 1 ? 'var(--color-surface-alt)' : 'transparent'; }}
					>
						{columns.map((c) => (
							<td
								key={c.key}
								style={{
									...
									fontWeight: c.strong ? 'var(--font-weight-semibold)' : 'var(--font-weight-regular)',
