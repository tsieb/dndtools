...
export function Sheet({
	...
	const panelShape = isBottom
		? { width: '100%', maxHeight: bottomHeight, borderRadius: 'var(--radius-xl) var(--radius-xl) 0 0', borderBottom: 'none', animation: 'dndSheetUp var(--duration-standard) var(--easing-decelerate)' }
		: side === 'right'
			? { width: sideWidth, maxWidth: '100%', height: '100%', borderRadius: 'var(--radius-xl) 0 0 var(--radius-xl)', borderRight: 'none', animation: 'dndSheetRight var(--duration-standard) var(--easing-decelerate)' }
			: { width: sideWidth, maxWidth: '100%', height: '100%', borderRadius: '0 var(--radius-xl) var(--radius-xl) 0', borderLeft: 'none', animation: 'dndSheetLeft var(--duration-standard) var(--easing-decelerate)' };

	return (
		<div
			style={{ position: 'fixed', inset: 0, zIndex: 'var(--z-sheet)', display: 'flex', ...align, background: 'var(--color-backdrop)', animation: 'dndScrimIn var(--duration-fast) var(--easing-standard)' }}
			onMouseDown={(e) => { if (dismissible && e.target === e.currentTarget) onClose && onClose(); }}
		>
			<style>{'@keyframes dndScrimIn{from{opacity:0}to{opacity:1}}@keyframes dndSheetUp{from{transform:translateY(100%)}to{transform:none}}@keyframes dndSheetRight{from{transform:translateX(100%)}to{transform:none}}@keyframes dndSheetLeft{from{transform:translateX(-100%)}to{transform:none}}'}</style>
			<div
				ref={panelRef}
				role="dialog"
				aria-modal="true"
			...
			>
				{isBottom && (
					<div aria-hidden="true" style={{ display: 'flex', justifyContent: 'center', paddingTop: 'var(--space-2)', flex: '0 0 auto' }}>
						<span style={{ width: 36, height: 4, borderRadius: 'var(--radius-full)', background: 'var(--color-border-strong)' }} />
					</div>
				)}
				{(title || dismissible) && (
					<div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-3)', padding: isBottom ? 'var(--space-3) var(--space-5) var(--space-4)' : 'var(--space-5)', borderBottom: '1px solid var(--color-border)' }}>
						<div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 'var(--space-1)' }}>
							{title && <h2 id={titleId} style={{ margin: 0, fontFamily: 'var(--font-sans)', fontSize: 'var(--text-lg)', fontWeight: 'var(--font-weight-semibold)', lineHeight: 1.25 }}>{title}</h2>}
				...
				{footer && (
					<div style={{ display: 'flex', alignItems: 'center', justifyContent: isBottom ? 'stretch' : 'flex-end', gap: 'var(--space-2)', flexWrap: 'wrap', padding: 'var(--space-3) var(--space-5)', borderTop: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>{footer}</div>
				)}
			</div>
		</div>
	);
