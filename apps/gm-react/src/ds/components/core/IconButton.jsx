...
export function IconButton({
	...
	return (
		<button
			...
			// `onMouseLeave` restores the RESTING appearance, which is the caller's `style` when it
			// overrode the variant — not the variant default. Restoring `v.color` unconditionally
			// permanently discarded a passed-in colour after the first hover: PlayerView's projected
			// stage runs its own theme and paints its Dismiss glyph `theme.ink`, so one hover flipped
			// it to `--color-text-secondary` on the surface the players are looking at, for good.
			onMouseEnter={(e) => {
				if (inert || variant === 'accent') return;
				e.currentTarget.style.background =
					variant === 'outline' ? 'var(--color-surface-overlay)' : 'var(--color-interactive-hover)';
