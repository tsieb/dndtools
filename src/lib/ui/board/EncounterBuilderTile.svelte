<script lang="ts">
	...
	function difficultyToneClass(
		...
		if (difficulty === 'trivial') return 'text-ink-muted';
	...
	<div class="flex-1 min-h-0 overflow-y-auto p-2.5 space-y-2.5">
		<section class="rounded border border-border/70 p-2">
			<div class="flex items-center justify-between gap-2">
				<p class="text-xs font-semibold text-ink">Difficulty Budget</p>
				<span class="text-xs font-semibold {difficultyToneClass(encounter.budget.difficulty)}">
					{encounter.budget.difficulty}
				</span>
			</div>
			<div class="mt-2 h-2 rounded bg-surface-alt overflow-hidden">
				<div
		...
		<section class="rounded border border-border/70 p-2 space-y-2">
			<div class="flex items-center justify-between gap-2">
				<p class="text-xs font-semibold text-ink">Environment</p>
				<select
		...
		<section class="rounded border border-border/70 p-2 space-y-2">
			<p class="text-xs font-semibold text-ink">Combatants (Stat Blocks)</p>
			<input
				type="text"
				bind:value={statBlockQuery}
				class="h-7 w-full rounded border border-border bg-surface px-2 text-xs"
			...
			{:else}
				<ul class="space-y-1.5">
					{#each encounter.combatants as entry (entry.id)}
						<li class="rounded border border-border/70 px-2 py-1.5 bg-surface-alt/50">
							<div class="flex items-center gap-2">
								...
								<label class="text-[11px] text-ink-muted">
									...
									<input
										...
										max="99"
										value={entry.count}
										class="ml-1 h-7 w-14 rounded border border-border bg-surface px-1.5 text-xs"
										onchange={(event) =>
