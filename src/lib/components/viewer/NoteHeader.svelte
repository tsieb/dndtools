<script lang="ts">
	...
	import { exportNote } from '$lib/services/export.js';
	...
	let filePath = $derived(
		note.filePath ??
			(note.folder === '/' ? `${note.title}.md` : `${note.folder.replace(/^\//, '')}/${note.title}.md`),
	);

	async function handlePin(): Promise<void> {
		await notesState.togglePin(note.id);
		toastState.success(note.pinned ? 'Note unpinned' : 'Note pinned');
	}
...
<div class="max-w-content mx-auto mb-6">
	<div class="flex items-start justify-between gap-4">
		<div class="min-w-0 flex-1">
			<h1 class="text-2xl font-bold text-ink dark:text-tavern-text break-words" style="font-family: var(--font-serif)">
				{note.title}
			</h1>
			<div class="flex items-center gap-2 mt-2 text-sm text-ink-muted dark:text-tavern-muted">
				<span class="px-2 py-0.5 rounded-md bg-surface-alt dark:bg-tavern-surface-alt text-xs font-mono truncate max-w-[420px]">
					{filePath}
				</span>
		...
		<div class="flex items-center gap-1 shrink-0">
			...
			>
				<svg class="w-4 h-4" fill={note.pinned ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
