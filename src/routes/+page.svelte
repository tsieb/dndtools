...
<div class="p-6 max-w-content mx-auto">
	{#if notesState.activeNotes.length === 0}
		<div class="text-center py-16">
			...
			<h1 class="text-3xl font-bold text-ink dark:text-tavern-text mb-3" style="font-family: var(--font-serif)">
			...
			<div class="flex items-center justify-center gap-3">
				...
				<a href={resolve('/settings')} class="text-sm text-ink-muted dark:text-tavern-muted hover:text-accent dark:hover:text-tavern-accent transition-colors">
					or explore settings
				</a>
			</div>
		</div>
	{:else}
		...
		<div class="mb-8">
			...
			<div class="grid grid-cols-3 gap-3 mt-4">
				<a href={resolve('/notes')} class="p-3 rounded-lg border border-border dark:border-tavern-border bg-surface dark:bg-tavern-surface hover:border-accent/40 dark:hover:border-tavern-accent/40 transition-all group">
					<div class="text-2xl font-bold text-accent dark:text-tavern-accent">{vaultState.noteCount}</div>
					<div class="text-xs text-ink-muted dark:text-tavern-muted group-hover:text-ink dark:group-hover:text-tavern-text transition-colors">Notes</div>
				</a>
				<div class="p-3 rounded-lg border border-border dark:border-tavern-border bg-surface dark:bg-tavern-surface">
		...
		{#if pinnedNotes.length > 0}
			<section class="mb-8">
				<div class="flex items-center gap-2 mb-4">
					<svg class="w-4 h-4 text-accent dark:text-tavern-accent" fill="currentColor" viewBox="0 0 24 24">
