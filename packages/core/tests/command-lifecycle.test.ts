...
describe('PLAT-018 AC2: undo only where the command contract supports it', () => {
	...
	it('every undoable mapping points at a real inverse command type', () => {
		const knownTypes = new Set([
			...
			'map.edit-layer',
			// MAP-021: every mutating map command is undoable through `buildMapInverse`.
			'map.add-features',
			'map.update-features',
			'map.remove-features',
			'map.generate',
			...
			'map.set-layer-enabled',
			'map.set-layer-opacity',
			'map.set-layer-tags',
			'map.create-poi',
			'map.update-poi',
			'map.delete-poi',
			...
			'map.append-fog',
			'map.remove-fog',
			'map.set-overlay-mode',
			'map.configure-overlay',
			'map.create-region',
			'map.update-region',
			'map.delete-region',
			'map.set-scale',
