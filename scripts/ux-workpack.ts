...
function uxGitWorkflow(): string[] {
	return [
		...
		'Before handoff, run targeted tests plus `pnpm ux-workpack:validate` and leave no stale generated UX planning diffs.',
	];
}
...
function uxStatusAutomation(epicId: string): string[] {
	return [
		...
		`Use \`pnpm ux-workpack:set-status -- --epic ${epicId} --status active\` when implementation starts.`,
		...
		`Use \`pnpm ux-workpack:complete -- --epic ${epicId}\` to update \`docs/planning/v2/ux/workpack-state.yaml\` and regenerate all derived UX planning files together.`,
		'Run `pnpm ux-workpack:validate` after any status or generation command.',
	];
}

function uxTestPlan(definition: UxEpicDefinition): string[] {
	const base = [
		'Run `pnpm ux-workpack:validate` before handoff.',
...
function defaultFileOwnership(definition: UxEpicDefinition): string[] {
	...
	if (definition.domain === 'UX-VIS' || definition.domain === 'UX-A11Y') {
		areas.push('apps/gm/src/routes/styles.css', 'apps/gm/src/lib/gui/');
		return areas;
	}
	areas.push(`apps/gm/src/lib/gui/${domain}/`, 'apps/gm/src/routes/');
	return areas;
}
...
function nextUxEpicStatusBlock(nextEpic: UxEpicPacket | null): Record<string, unknown> | null {
	...
	return {
		...
		promptCommand: `pnpm ux-prompt -- --epic ${nextEpic.id}`,
		nextPromptCommand: 'pnpm ux-prompt -- --next',
		statusCommand: `pnpm ux-workpack:set-status -- --epic ${nextEpic.id} --status active`,
	};
}
...
function compareGeneratedFile(
	...
	return {
		...
		message: 'Generated UX workpack file is stale. Run pnpm ux-workpack:generate.',
	};
}
	...
	for (const [file, expected] of expectedFiles) {
		try {
			...
			if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
				issues.push({
					file: path.relative(root, file).replace(/\\/g, '/'),
					message: 'Generated UX workpack file is missing. Run pnpm ux-workpack:generate.',
				});
				continue;
			}
			throw error;
...
async function validateUxCompletionEvidence(
	...
	for (const evidenceFile of await collectUxCompletionEvidenceFiles(root)) {
		...
		issues.push({
			...
			message: `UX completion evidence exists but epic status is ${status}. Run pnpm ux-workpack:complete -- --epic ${epicId}.`,
		});
	}
	return issues;
}
...
export async function validateUxWorkpack(
	...
	if (!state) {
		issues.push({
			...
			message: 'Missing mutable UX workpack state. Run pnpm ux-workpack:generate.',
		});
	}
	...
	if (epicFiles.length === 0) {
		issues.push({
			...
			message: 'No UX epic YAML files found. Run pnpm ux-workpack:generate.',
		});
	}
...
async function loadEffectiveUxWorkpack(root = repoRoot): Promise<{
	...
	if (!state) {
		throw new Error(
			`Missing docs/planning/v2/ux/${uxStateFileName}. Run pnpm ux-workpack:generate.`,
		);
	}
...
async function runPrompt(args: string[]): Promise<void> {
	...
	if (!epicId && !useNext) {
		throw new Error('Usage: pnpm ux-prompt -- --epic <epic-id> or pnpm ux-prompt -- --next');
	}
...
async function runSetStatus(args: string[]): Promise<void> {
	const epicId = getFlagValue(args, '--epic');
	if (!epicId) {
		throw new Error('Usage: pnpm ux-workpack:set-status -- --epic <epic-id> --status <status>');
	}
...
async function runComplete(args: string[]): Promise<void> {
	const epicId = getFlagValue(args, '--epic');
	if (!epicId) {
		throw new Error('Usage: pnpm ux-workpack:complete -- --epic <epic-id> [--evidence <file>]');
	}
...
async function main(): Promise<void> {
	...
	throw new Error(
		'Usage: pnpm ux-workpack:<generate|validate|status|next|prompt|set-status|complete> or pnpm ux-prompt -- --epic <id|--next>',
	);
}
