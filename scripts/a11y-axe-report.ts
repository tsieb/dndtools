 ...
 * (`apps/v2/app/tests/e2e/a11y-axe-gate.spec.ts`), merges and de-duplicates them, applies the
 ...
 *     --artifacts apps/v2/app/test-results/a11y \
 *     --register apps/v2/app/tests/a11y/known-violations.json \
 *     --out tmp/a11y
 */
...
function main(): void {
	...
	const artifactsDir = resolve(REPO_ROOT, args.artifacts ?? 'apps/v2/app/test-results/a11y');
	const registerPath = resolve(
		REPO_ROOT,
		args.register ?? 'apps/v2/app/tests/a11y/known-violations.json',
	);
