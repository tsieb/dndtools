...
function inferAffectedAreas(domain: string): string[] {
	const shared = ['apps/gm', 'docs/planning/v2'];
...
function statusAutomation(epicId: string): string[] {
	return [
		...
		`Use \`pnpm workpack:set-status -- --epic ${epicId} --status active\` when implementation starts.`,
		...
		`Use \`pnpm workpack:complete -- --epic ${epicId}\` to update \`workpack-state.yaml\` and regenerate all derived planning files together.`,
		'Run `pnpm workpack:validate` after any status or generation command; validation fails if generated files drift from the source of truth.',
	];
}
...
export function buildEpicPackets(pack: RequirementPackage): EpicPacket[] {
	...
	for (const branch of pack.branches) {
		...
		epics.push({
			...
			dependencies: [],
			parallelSafety: {
				fileOwnership: [`apps/gm/${branch.domain.toLowerCase()}/`, 'docs/planning/v2/'],
			...
			testPlan: [
				...
				'Run `pnpm workpack:validate` before handoff.',
			],
			demoNotesTemplate: [
...
function nextEpicStatusBlock(nextEpic: EpicPacket | null): Record<string, unknown> | null {
	...
	return {
		...
		promptCommand: `pnpm prompt -- --epic ${nextEpic.id}`,
		nextPromptCommand: 'pnpm prompt -- --next',
		statusCommand: `pnpm workpack:set-status -- --epic ${nextEpic.id} --status active`,
	};
}
...
function compareGeneratedFile(
	...
	return {
		...
		message: 'Generated file is stale. Run pnpm workpack:generate.',
	};
}
	...
	for (const [file, expected] of expectedFiles) {
		try {
			...
			if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
				issues.push({
					file: path.relative(root, file).replace(/\\/g, '/'),
					message: 'Generated file is missing. Run pnpm workpack:generate.',
				});
				continue;
			}
			throw error;
...
async function validateCompletionEvidence(
	...
	for (const evidenceFile of await collectCompletionEvidenceFiles(root)) {
		...
		issues.push({
			...
			message: `Completion evidence exists but epic status is ${status}. Run pnpm workpack:complete -- --epic ${epicId}.`,
		});
	}
	return issues;
}
...
export async function validateWorkpack(root = repoRoot): Promise<WorkpackValidationIssue[]> {
	...
	if (!state) {
		issues.push({
			...
			message: 'Missing mutable workpack state. Run pnpm workpack:generate.',
		});
	}
	...
	if (epicFiles.length === 0) {
		issues.push({
			...
			message: 'No v2 epic YAML files found. Run pnpm workpack:generate.',
		});
	}
...
async function loadEffectiveWorkpack(root = repoRoot): Promise<{
	...
	if (!state) {
		throw new Error(
			`Missing docs/planning/v2/${workpackStateFileName}. Run pnpm workpack:generate.`,
		);
	}
...
async function runPrompt(args: string[]): Promise<void> {
	...
	if (!epicId && !useNext) {
		throw new Error('Usage: pnpm prompt -- --epic <epic-id> or pnpm prompt -- --next');
	}
...
async function runSetStatus(args: string[]): Promise<void> {
	const epicId = getFlagValue(args, '--epic');
	if (!epicId) {
		throw new Error('Usage: pnpm workpack:set-status -- --epic <epic-id> --status <status>');
	}
...
async function runComplete(args: string[]): Promise<void> {
	const epicId = getFlagValue(args, '--epic');
	if (!epicId) {
		throw new Error('Usage: pnpm workpack:complete -- --epic <epic-id> [--evidence <file>]');
	}
...
async function main(): Promise<void> {
	...
	throw new Error(
		'Usage: pnpm workpack:<generate|validate|status|next|set-status|complete> or pnpm prompt -- --epic <id|--next>',
	);
}
