...
async function main() {
	...
	try {
		...
		bad('host advertises a session', e.message);
	}
	...
	try {
		...
		if (services.services?.some((s) => s.sessionId === sessionId))
			ok('client browse lists the session');
		else bad('client browse lists the session', 'session absent from list');
	} catch (e) {
		bad('client browse lists the session', e.message);
	}

	// 6. Client joins → host receives an offer-request.
	send(client, { action: 'join', sessionId, pubKey: ephemeralPublicKey() });
	let reqId;
	try {
		...
		bad('client join reaches host as offer-request', e.message);
	}

	// 7. Host responds with an offer → client receives it.
	if (reqId) {
		send(host, { action: 'offer', reqId, offerCode: 'OFFER-BLOB', pubKey: ephemeralPublicKey() });
		try {
			...
			bad('host offer relays to client', e.message);
		}
		...
		try {
			...
			bad('client answer relays to host', e.message);
		}
	}
	...
	try {
		...
		if (relay?.username && relay?.credential)
			ok('mints TURN credentials (username + HMAC credential)');
		else bad('mints TURN credentials', 'missing username/credential');
	} catch (e) {
		bad('mints TURN credentials', e.message);
	}
