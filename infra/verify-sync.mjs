...
const VAULT = process.env.VAULT_ID ?? 'primary';
const headers = { authorization: `Bearer ${TOKEN}`, 'content-type': 'application/json' };
const b64urlBytes = (s) => Math.floor((s.length * 3) / 4);
const tokenSegment = TOKEN.split('.')[1];
const accountId = tokenSegment
	? JSON.parse(Buffer.from(tokenSegment, 'base64url').toString('utf8')).sub
	: null;
if (typeof accountId !== 'string' || accountId.length === 0) {
	console.error('TOKEN must be a Cognito ID token with a non-empty sub claim');
	process.exit(2);
}
...
async function main() {
	...
	const operationContext = { accountId, vaultId: VAULT, kind: 'operation', revision: 0 };
	const envelope = await encryptForKeyring(keyring, op, operationContext);
	const meta = {
	...
	if (
		findServerVisibilityViolations(
			DNDTOOLS_CLOUD_SECURITY_DECISION_RECORD,
			opServerVisibleFields(VAULT, meta),
		).length === 0
	)
	...
	const pushRes = await fetch(`${SYNC_URL}/vaults/${VAULT}/operations`, {
		method: 'POST',
		headers,
		body: JSON.stringify({ ops: [{ meta, envelope }] }),
	});
	const pushBody = await pushRes.json().catch(() => ({}));
	if (pushRes.ok && Array.isArray(pushBody.accepted) && pushBody.accepted.includes(0))
		ok('pushed an encrypted operation (accepted)');
	else
		bad(
			'pushed an encrypted operation',
			`status=${pushRes.status} body=${JSON.stringify(pushBody)}`,
		);
	...
	if (pulled?.envelope) {
		...
		if (!ctText.includes(SECRET) && !ctText.includes('Bearer'))
			ok('the stored ciphertext does NOT contain the plaintext secret');
		else bad('the stored ciphertext leaks the plaintext secret');

		const decrypted = await decryptFromKeyring(keyring, pulled.envelope, operationContext);
		if (JSON.stringify(decrypted) === JSON.stringify(op))
			ok('decrypted operation matches the original (round-trip)');
	...
	const snapshotContext = { accountId, vaultId: VAULT, kind: 'snapshot', revision: 1 };
	const snapEnv = await encryptForKeyring(keyring, slice, snapshotContext);
	const snapMeta = {
		revision: 1,
		size: b64urlBytes(snapEnv.ct),
		contentHash: snapEnv.contentHash,
		issuedAt: new Date().toISOString(),
	};
	const putRes = await fetch(`${SYNC_URL}/vaults/${VAULT}/snapshot`, {
		method: 'PUT',
		headers,
		body: JSON.stringify({ meta: snapMeta, envelope: snapEnv }),
	});
	if (putRes.ok) ok('pushed a full-state snapshot');
	else bad('pushed a full-state snapshot', `status=${putRes.status}`);

	const getRes = await fetch(`${SYNC_URL}/vaults/${VAULT}/snapshot/latest`, { headers });
	const getBody = await getRes.json();
	if (getRes.ok && getBody.envelope) {
		const restored = await decryptFromKeyring(keyring, getBody.envelope, snapshotContext);
		if (JSON.stringify(restored) === JSON.stringify(slice))
			ok('restored snapshot matches (fresh-device path)');
	...
	const evilRes = await fetch(`${SYNC_URL}/vaults/${VAULT}/operations`, {
		method: 'POST',
		headers,
		body: JSON.stringify({ ops: [{ meta: evilMeta, envelope }] }),
	});
	if (evilRes.status === 400)
		ok('server rejects plaintext smuggled into a metadata field (fail closed)');
